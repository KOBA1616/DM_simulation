...
class DeckBuilder(QWidget):
    ...
    def save_deck(self):
        if len(self.current_deck) != 40:
            QMessageBox.warning(self, "Invalid Deck", "Deck must have exactly 40 cards.")
            return
            
        os.makedirs("data/decks", exist_ok=True)
        fname, _ = QFileDialog.getSaveFileName(self, "Save Deck", "data/decks", "JSON Files (*.json)")
        if fname:
    ...
    def load_deck(self):
        os.makedirs("data/decks", exist_ok=True)
        fname, _ = QFileDialog.getOpenFileName(self, "Load Deck", "data/decks", "JSON Files (*.json)")
        if fname:
            try:
