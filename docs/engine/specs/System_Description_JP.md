...
4. **Transformerによる文脈・シナジー理解**: MLPの限界を超え、可変長の盤面情報とカード間の相互作用（Synergy）を捉えることで、対戦相手の意図予測やコンボ形成能力を向上させる（Phase 4）。
5. **実験の反復可能性の確保**: 設定・ログ・評価指標を明確化し、モデルや探索パラメータの変更が性能に与える影響を追跡できる状態を維持する。
...
AI機能の実装における基本方針は「探索を強くする前に、探索を成立させる」ことである。具体的には、(1) まずルールに忠実な合法手生成を前提に分岐の原因を分析し、(2) 次にCPUでの許容計算量を基準として探索の上限（ノード数・深さ・候補数）を固定し、(3) 最後に Transformer の Self-Attention および Synergy Bias により候補を精緻に優先度付けして、上限内でより良い手を探索できるようにする。
...
## 7. 技術的展望：Phase 4 Transformerアーキテクチャ
現在進行中の第4フェーズ（Phase 4）では、従来のMLP（多層パーセプトロン）ベースの限界（可変長状態の表現力不足、カード間相互作用の欠落）を克服するため、Transformer Encoder を中核とした次世代推論エンジンの実装を進めている。

### 7.1 アーキテクチャ仕様（Architecture Specs）
**文書ID**: REQ-PHASE4-TRANSFORMER-001 に基づき、以下の仕様で実装される。

- **モデル構造**: BERT/GPT-2 準拠の Encoder-only 構成
- **次元設定**: $d_{model}=256, n_{head}=8, n_{layers}=6$（計算コストと表現力のバランス）
- **入力長**: Max Sequence Length $200$ token
- **語彙サイズ**: $1000$（カードID + 特殊トークン）

#### 入力トークン表現
ゲーム状態を固定長ベクトルではなく、ゾーンごとの意味的まとまりを持つトークン列として表現する。

```cpp
[CLS] [GLOBAL] [SEP] 
[SELF_HAND] card_A card_B ... [SEP] 
[SELF_MANA] card_C ... [SEP] 
[OPP_BATTLE] card_X ... [SEP]
... 
[PAD] ...
```

この表現により、カードのプレイ順序やゾーン内の隣接関係（進化元・進化先など）を構造的に保持することが可能となる。

### 7.2 Synergy Bias による知識注入
Attention Mechanism に対し、カード間の既知の相性（コンボ）をバイアスとして注入する **Synergy Bias** を導入する。

$$
\text{Attention}(Q, K, V) = \text{softmax}\left(\frac{QK^T}{\sqrt{d_k}} + M_{\text{synergy}}\right)V
$$

ここで行列 $M_{\text{synergy}}$ は、例えば「革命チェンジ元」と「革命チェンジ先」のペアに対して正の値を持ち、学習初期段階から有効な相互作用への着目を促進する。これは、探索数が限られるCPU環境において、有望な手を早期に発見させるための重要な帰納バイアス（Inductive Bias）となる。

### 7.3 今後のロードマップ
1. **データパイプライン構築**: C++エンジンからのトークン列出力（`convert_to_sequence`）とPython側 `DataLoader` の結合。
2. **学習ループの確立**: AlphaZero形式の強化学習ループへの統合（`TransformerTrainer`）。
3. **軽量化と蒸留**: CPU推論レイテンシ $< 10$ms を達成するため、学習済みモデルからの知識蒸留（Distillation）や量子化の適用。
4. **品質保証**: GUIスタブやテキスト生成等の残課題解消による実験環境の安定化。
