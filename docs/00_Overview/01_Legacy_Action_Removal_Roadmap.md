...
## ステータス
*   [Status: Done] Phase 1: コマンド基盤の完成と変換レイヤーの整備
...
1.  **GameCommandの拡充**:
    ...
    *   不足リスト: `SearchDeck`, `ShuffleDeck`, `ShieldTrigger`, `SelectTarget` (汎用), `ResolveBattle` (詳細化) 等。
    *   `src/core/game_command.hpp` および `src/engine/game_command.cpp` を更新。
    *   **完了 (Done)**: `ShuffleCommand`, `SearchDeckCommand`, `ShieldTriggerCommand`, `ResolveBattleCommand` を実装・Pythonバインディング公開済み。
2.  **Conversion Layer**:
...
ユーザー指示「要件定義書01に従い、旧アクションからコマンド方式への移行を進めて...」に基づき、以下を実行した。

1.  不足している `GameCommand` の実装 (`src/core/game_state_command.cpp` および `src/engine/game_command/action_commands.hpp/cpp`).
    *   Implemented: `ShuffleCommand`, `SearchDeckCommand`, `ShieldTriggerCommand`, `ResolveBattleCommand`.
2.  Python Binding の更新 (`src/bindings/bind_engine.cpp`)。
3.  検証テストの実施。
