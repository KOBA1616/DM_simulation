...
1.  **革命チェンジのデータ構造不整合**
    ...
    *   effect設定画面の日本語化、それぞれのタブの日本語化、プルダウンの選択肢の日本語化を進めてください。
    *   カードの縁(赤と青の文明を選択したときに紫色に変わる部分)をすべて黒の細線にしてください。
    *   ツインパクトカードのパワー表記は半分より上の左下ではなく、カードの左下に配置してください。
    *   マナコストの丸は文明の色で構成され、多色の場合は等分割される。また、文字色は細い黒縁のある白文字とする。
