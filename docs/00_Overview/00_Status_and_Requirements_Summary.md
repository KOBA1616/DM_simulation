...
**ステータス:** **GUI拡張と汎用エンジン機能の統合**
現在、以下の開発に注力しています。
1.  **GUI拡張（視覚的ビルダ）:** `card_editor.py` における「視覚的ビルダ」機能を強化し、ユーザーが直感的に複雑な効果（トリガー条件、対象指定、アクション）を構築できるようにします。革命チェンジ等の新機能のUI対応も完了し、さらなる複雑な効果（サーチ、モード選択）の対応を進めています。
2.  **汎用エンジン機能の統合:**
    *   **サーチ系の統合:** `SEARCH_DECK_BOTTOM` などの複合アクションを、`LOOK` (見る) -> `SELECT` (選ぶ) -> `MOVE` (移動する) という原子アクションの組み合わせに分解し、汎用性を高めます。
    *   **ゾーン移動の完全統合:** 様々な移動アクションを `MOVE_CARD` に一元化し、処理の一貫性を確保します。
...
### 4.3. 開発中の汎用エンジン機能 (PLAN-002残り)

#### 革命チェンジ (完了済み)
*   **現状:** 専用アクション `REVOLUTION_CHANGE` とトリガー `ON_ATTACK_FROM_HAND` の基本ロジックは実装済み。GUIエディタからの設定も可能。
