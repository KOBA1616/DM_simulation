...
2.  **カードエディタ GUI 改善 (Card Editor GUI Improvements)**
    ...
    *   **GUI改善**
    *   アクションタイプの日本語化
        *   *ステータス: 完了*。`localization.py` を更新し、主要なアクションタイプ・トリガー・ゾーン名の日本語化マッピングを追加しました。
    *   選択したアクションタイプによって、必要なフィルタ以外をマスク
    *   カードプレビュー画面では、背景色を文明の色および、グラデーションにする。
    *   カードプレビュー画面の縁の線の太さをテキストゾーンの枠と同じにする
    *   丸の背景を背景色と同じにする。
    *   ツインパクト時のプレビュー対応を進める(カードの左下にパワー表記)
    *   グラデーションの色をそれぞれ少し濃くする。
        *   *ステータス: 完了*。`preview_pane.py` を更新し、文明色に基づくグラデーション（単色・多色）、縁取り、コスト背景の透過、ツインパクト時のパワー表記調整を実装しました。
    *   能力のトリガータイミングの日本語化
    ...
    *   キーワード能力チェックボックス：メタカウンターの削除
    *   デッキは一つの束として扱い盤面に表示するときは、単一の不明カードとして表示で良い
    *   アクションでマナゾーンの文明の数を数える+カードを引く(任意)+同じ枚数(ドローの出力)を手札から山札の下に置くという一連のアクションから、プレビューで、マナゾーンの文明1つにつきカードを1枚引いてもよい、その後、引いた枚数と同じ枚数の手札を、好きな順番で山札の下に置く。のように日本語化したい。(内部の処理では戻す枚数だけ、手札から1枚選択して、山札の下に置くを繰り返してもよい)

    *   **システム改善**
    *   効果バッファ (effect_buffer) のプレイヤー別分離（推奨）
        *   *ステータス: 完了*。C++コアエンジンにて、`effect_buffer` を `GameState` から `Player` 構造体へ移動し、プレイヤーごとの独立したバッファリングを実現しました。これにより、S・トリガーの割り込みや入れ子のアクション処理時におけるデータ破損（バッファ混線）を完全に防止しました。関連するハンドラ (`BufferHandler`, `MoveCardHandler`, `GenericCardSystem`) もすべて更新済みです。
    *   アンタップインのキーワード能力が間違っている。タップして出ると表示されているが、手札からマナゾーンに置かれたときにアンタップする効果に変更
    *   ツインパクトカードモードを選択したとき、左のツリーに呪文のeffectとアクションを自動的に生成し、クリーチャーの部分と接続することで視覚的にわかりやすい木構造にする。
