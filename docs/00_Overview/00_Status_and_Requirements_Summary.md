...
1.  **革命チェンジのデータ構造不整合**
    ...
    *   キーワード能力：エクストライフの追加。（完了）
    *   シールドもデッキと同様に塊で表記して枚数を表示。（完了）
    *   トリガー：「呪文を唱えた時」を追加。（完了）
    *   ツインパクトカードでも、エフェクトを追加するとき、クリーチャーの葉の直下に追加(呪文側が自動スライド)。（完了）
    *   右上にプレビュー更新ボタンを追加。（完了）
...
2.  **Kaggle環境最適化とC++移行 (Kaggle Optimization & C++ Migration)**
    ...
    *   **Step 1: デッキ進化ロジックのC++化**
        *   `dm_toolkit/training/verify_deck_evolution.py` のPythonスタブ実装を廃止し、既に実装済みのC++モジュール `dm_ai_module.DeckEvolution` を使用するように修正済み（完了）。
