...
2.  **カードエディタ GUI 改善 (Card Editor GUI Improvements)**
    ...
    *   アクションタイプの日本語化
        *   *ステータス: 完了*。`localization.py` にアクションタイプの日本語訳を追加しました。
    *   カードプレビュー画面では、背景色を文明の色および、グラデーションにする。
    *   グラデーションの色をそれぞれ少し濃くする。
    *   カードプレビュー画面の縁の線の太さをテキストゾーンの枠と同じにする
    *   丸の背景を背景色と同じにする。
        *   *ステータス: 完了*。`preview_pane.py` を更新し、グラデーション（濃いめ）、枠線（1px）、丸背景（透明/同色）の実装を行いました。
    *   進化はキーワード能力ではなくカードタイプである。
    *   キーワード能力チェックボックス：メタカウンターの削除
        *   *ステータス: 完了*。`CardEditForm` のキーワードリストから `evolution` と `meta_counter_play` を削除しました。

    *   **システム改善**
    *   効果バッファ (effect_buffer) のプレイヤー別分離（推奨）
        *   *ステータス: 完了*。`GameState` から `effect_buffer` を削除し、`Player` 構造体に `effect_buffer` を移動させました。これにより、プレイヤーごとのバッファ管理が可能となり、入れ子処理時のデータ破損リスクが解消されました。
    *   アンタップインのキーワード能力が間違っている。タップして出ると表示されているが、手札からマナゾーンに置かれたときにアンタップする効果に変更
        *   *ステータス: 完了*。`localization.py` の表示テキストを「マナゾーンに置く時アンタップして置く」に修正しました。ロジック自体は既存の `TapInUtils` により正しく「多色タップインを無効化（＝アンタップイン）」する挙動となっています。
