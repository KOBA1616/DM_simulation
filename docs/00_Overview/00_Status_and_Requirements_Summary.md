...
3.  **今後の課題 (Future Issues/Known Bugs)**
    ...
    *   **フレンド・バーストの検証失敗 (Friend Burst Verification Failure)**: `test_friend_burst.py` が失敗する。`USE_ABILITY` アクションが生成されない問題が残存しており、C++側の `MainPhaseStrategy` ロジックまたはデータ連携（Spell Side / Keywords）の調査が必要です。
...
2.  **エンジン・ロジック拡張**
    *   **フレンド・バースト実装 [実装済み/要デバッグ]**:
        *   `CardKeywords` に `friend_burst` を追加。
        *   `MainPhaseStrategy` にアクション生成ロジックを実装。
        *   `FriendBurstHandler` による処理（タップ＆呪文詠唱）を実装。
        *   ※ 検証テスト `tests/test_friend_burst.py` が失敗中。
    *   **新規アクション・トリガー追加 [実装済み]**:
        *   **アクション**: `CAST_SPELL_FROM`（呪文を唱える）、`PUT_CREATURE`（クリーチャーを出す）、`GRANT_KEYWORD`（キーワード付与）を追加。
        *   **トリガー**: `ON_OPPONENT_CREATURE_ENTER`（相手獣着地時）、`ON_OPPONENT_DRAW_SECOND`（相手ターン2枚目ドロー時）を追加。
    *   **ロック能力の汎用化**: `LOCK_SPELL_BY_COST` を「コスト指定ロック」として汎用化・再定義する（未着手）。
    *   **新規アクション・ロジック追加**:
        *   ~~**呪文を唱えるアクション**: ゾーン（どこから）、コスト、文明を指定して呪文を唱えるアクションを追加。~~ [完了]
        *   ~~**クリーチャーを出すアクション**: 召喚扱いではない「出す」処理のアクションを追加。~~ [完了]
        *   **キーワード能力付与**: 任意対象にキーワード能力を付与するアクションを追加（ロジック実装済み、GUI未確認）。
