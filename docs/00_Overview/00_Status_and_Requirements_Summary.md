...
*   ツインパクトカードにおいて、唱えた時の効果とバトルゾーンに出た時の効果をそれぞれの側で設定できるようにしつつ、木構造の表示形式をわかりやすくしてください。（完了）
...
*   アクションのプルダウンの中身も日本語化を推進（進行中）
*   選択したアクションによって必要な部分のみを有効化し、そのほかをマスクする。（完了）
*   右側のカードプレビュー画面のレイアウトを調整し、generated textとカードプレビューが適切な配置になるように調節してください。（完了）
