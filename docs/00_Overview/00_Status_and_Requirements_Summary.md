...
3.  **アクション汎用化**
    *   **Status**: `MOVE_CARD`、`TAP`、`UNTAP`、`APPLY_MODIFIER`、`GRANT_KEYWORD` のハンドラを `GameCommand` を使用するように移行完了 (Phase 6.3 Ongoing)。
    *   **Note**: `APPLY_MODIFIER` および `GRANT_KEYWORD` は `MutateCommand` に `ADD_MODIFIER` / `ADD_PASSIVE` を追加して対応。
    *   **Next**: 残りのアクション (`MODIFY_POWER` 等) の移行。
