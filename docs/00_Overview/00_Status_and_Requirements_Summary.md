...
**ステータス:** **GUI機能拡張（フィルタリング詳細化）完了**

ジャストダイバーの実装に続き、GUIツール（カードエディタ）の機能を大幅に拡張しました。これにより、JSONを直接編集することなく、文明・種族・コスト/パワー範囲・状態（タップ/ブロッカー/進化）を含む詳細なフィルタリング条件を設定可能になりました。
...
*   **カード作成補助 (日本語化 & 視覚化):**
    ...
    *   **視覚的ビルダ (進行中):**
        ...
        *   **残り:** トリガー条件(Condition)のGUIエディタ、コスト軽減の専用UIなど。
