...
2.  **ツインパクトカードの視覚的統合**
    ...
    *   **実装内容**:
        *   `CardPropertiesWidget` を作成し、カード属性入力部分をモジュール化。
        *   `CardEditForm` にツインパクト切り替えチェックボックスと `QSplitter` を導入し、クリーチャー側と呪文側の同時編集を実現。
        *   `creature_part`（ルート要素）と `spell_part`（`spell_side`要素）のデータ構造へのマッピングを実装し、カード名の自動結合（Name / Name）機能を導入。
