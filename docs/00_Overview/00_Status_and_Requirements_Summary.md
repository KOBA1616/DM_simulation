...
1.  **エンジン・ロジック拡張**
    *   **フレンド・バースト実装**: アクションに追加。内部処理用IDを自動割り振りし、ユーザーに意識させずに処理する。
        *   *ステータス: 実装完了（検証中）*。`CardKeywords` に `friend_burst` フラグを追加し、JSONからの読み込み、Pythonバインディング、およびエンジン内でのアクション生成・解決ロジックを実装しました。
        *   *課題*: 検証用テスト `tests/test_friend_burst.py` がフェーズ不一致またはバインディングの問題で失敗しており、修正が必要です。
    ...
    *   ツインパクトカードも上下面でそれぞれ文明を指定できるようにする。
        *   *ステータス: 進行中*。`CardTextGenerator`を更新し、`FRIEND_BURST` などの新規キーワードおよびアクションの日本語生成に対応しました。
        *   *未実装課題*: `CardPreviewWidget` の描画ロジック修正（パワー位置、枠線色、マナコスト描画）は未完了のため、次回のタスクとして実施します。
...
2.  **シナリオエディタの機能改善 (Scenario Editor Improvements)**
    *   **ゾーン管理の構造化**: 各ゾーン（Hand, Battle Zone, Mana Zone, Shield Zone, Graveyard）をタブまたはグループで整理し、カードID配列ではなく「カード名のリスト」として管理します。
        *   *ステータス: 実装完了*。タブ管理（`QTabWidget`）を導入し、General設定と各ゾーン（テキスト入力形式）を分離しました。
    *   **カード検索・追加機能**: カード名、文明、コスト、テキストでの検索・フィルタリング機能を実装します。
        *   *ステータス: 実装完了*。カード検索ウィジェット (`CardSearchWidget`) を実装し、名前、文明、コストによるフィルタリングを可能にしました。
    *   **ドラッグ＆ドロップ**: 検索結果リストから各ゾーンへ直感的にカードを追加できるようにします。
        *   *ステータス: 実装完了*。ゾーン編集エリアを `DraggableListWidget` に置き換え、検索結果からのドラッグ＆ドロップによるカード追加に対応しました。
...
3.  **カードエディタ GUI 改善 (Card Editor GUI Improvements)**
    ...
    *   **ツインパクト対応**:
        ...
        *   *ステータス: 実装完了*。`CardPreviewWidget` にツインパクト用のスプリットビュー（上下分割レイアウト）を追加し、各パートの表示を実装しました。
...
4.  **Game Info ウィンドウの整理**
    *   **レイアウト分離**:
        *   **上部**: ゲーム進行操作、必須情報。
        *   **下部**: AI設定、シミュレーション設定。
        *   *ステータス: 実装完了*。`GameWindow` のレイアウトを再設計し、上部にゲーム操作・ステータス、下部にAI設定・ツール群を分離したグループボックス配置に変更しました。
