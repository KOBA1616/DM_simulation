...
今後は、AIが自律的に進化するための「エコシステム」と、より高度な探索アルゴリズムの統合を目指します。また、直近のユーザーフィードバックに基づき、GUIとエンジンの整合性確保およびリファクタリングを優先的に実施します。
...
2.  **重要バグ修正 (Critical Bug Fixes)**
    ...
    *   **GUI起動エラー修正 (New)**: `dm_toolkit/gui/app.py` で発生している `AttributeError: type object 'dm_ai_module.PhaseManager' has no attribute 'check_game_over'` を修正します。バインディング側でのメソッド公開漏れ対応を行います。
    *   **MCTS可視化機能の復旧 (New)**: コンパイル安定化のために一時的に無効化された `MCTSNode` のPythonバインディングを、`unique_ptr` を考慮した安全な形で再実装し、GUI上での探索ツリー可視化機能を復旧させます。

3.  **アーキテクチャ改善 (Architecture Improvements)**
    *   **CardInstance所有者情報の追加 (New)**: `src/core/types.hpp` の `CardInstance` 構造体に `owner` フィールドを追加します。現状 `GameState` の `card_owner_map` に依存している所有者判定を改善し、コードの可読性とテストの容易性を向上させます。同期コストとのトレードオフを考慮して実装します。
    *   **GUIシミュレーションバックエンドの統一 (New)**: GUIの「バッチシミュレーション」機能を、独自のループ実装から `dm_ai_module.ParallelRunner` をバックエンドとして利用するように変更し、コードの重複排除と高速化を実現します。

4.  **機能改善・拡張 (Feature Improvements)**
...
3.  **デッキ進化 (Deck Evolution)**
    ...
    *   **実装計画**:
        *   **ロジック**: カードごとの「勝率貢献度（Win Contribution）」を算出し、貢献度の低いカードを候補プール（`candidate_pool`）のカードと入れ替える遺伝的アルゴリズムを実装。
        *   **C++移植による高速化 (New)**: `DeckEvolution` クラスおよび `calculate_interaction_score` ロジックを C++ (`src/ai/evolution/`) に移植し、`dm_ai_module` から呼び出し可能にすることで、Pythonでの計算ボトルネックを解消します。
...
### 3.5 [Priority: High] User Requested Enhancements (ユーザー要望対応)

直近のフィードバックに基づき、ユーザビリティ向上とロジックの汎用化を行います。

1.  **カードエディタ UI/UX 改善**
    *   **レイアウト刷新**: ウィンドウ横幅をフル活用し、`| 木構造ビュー | 設定画面 | 生成テキストプレビュー |` の3ペイン構成とする。
    *   **プレビュー機能強化**: 生成テキストプレビュー領域に、シミュレーション盤面で使用されるカードと同じような「簡易カード外観プレビュー」を表示する。
    *   **文明選択UI**: ボタンの縁を各文明色で装飾する。選択なしを「ゼロ文明」と見なし、ゼロ文明ボタンは削除する。
    *   **入力フィールド制御**: カードタイプ「呪文」選択時はパワー入力欄をGUI上でマスクする。
    *   **テキスト生成検証**: 設定されたアクション・能力からカードテキストが正しく生成されることを検証する。

2.  **シナリオエディタの改善 (Scenario Editor Improvements)** (New)
    *   **ゾーン構成の整理**: 各ゾーン（手札、バトルゾーン、マナゾーン、シールド、墓地）をタブ (`QTabWidget`) またはグループボックスで整理し、視認性と操作性を向上させます。
    *   **カードリスト管理**: カードIDの羅列ではなく、ユーザーが識別しやすい「カード名のリスト」として管理します。
    *   **検索・追加機能の実装**:
        *   **検索ダイアログ**: カード名、文明、コスト、テキストでのフィルタリングが可能な検索画面。
        *   **ドラッグ＆ドロップ**: 検索結果から各ゾーンのリストへドラッグ＆ドロップでカードを追加できる機能。
    *   **操作性**: 各リストに対する「追加」「削除」「クリア」ボタンの整備。

3.  **Game Info ウィンドウの整理**
    *   **レイアウト分離**:
        *   **上部**: ゲーム進行操作、必須情報。
        *   **下部**: AI設定、シミュレーション設定。
...
### 3.6 [Priority: Next] 今後の課題 (Future Tasks)

今回の基盤安定化フェーズの次に優先的に取り組むべき課題です。

1.  **リーサルソルバーの高度化**
    *   今回は基盤安定化を優先したため未着手ですが、除去、タップ、ブロック不可などを考慮した「動的なブロッカー数変動」に対応する詰め将棋的な勝利判定ロジックの実装を行います。
2.  **テストカバレッジの拡充**
    *   新規実装した「呪文ロック効果」などについて、エンジンの内部ロジックだけでなく、JSONデータ定義を含めたエンドツーエンドのテストケースを作成し、品質を担保します。
