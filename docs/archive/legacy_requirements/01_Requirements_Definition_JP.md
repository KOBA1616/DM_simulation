...
## 10. 変更履歴
- 2026-02-04: タスクA, Bの完了を確認し、ドキュメントを更新。アーカイブへの移動を実施。
...
- 優先対応（短期）
	- GUIスタブとヘッドレステストの完全安定化：`tests/test_compat_phase_manager.py` 等で一部残るスタブ起因エラーを解消する。
	- ドキュメント整理：`docs/` のアーカイブ化を完了する（本更新で対応済み）。
...
- [Status: Done] **タスク A: `dm_toolkit.gui.i18n` の堅牢化**
	- Enumでないオブジェクトを検出してスキップし、初期化でクラッシュしないように修正済み。

- [Status: Done] **タスク B: ルート整理のレビュー & リファクタ**
	- 開発者向けの参照を `docs/` `scripts/` に一本化し、古いファイルを `docs/archive/` へ移動済み。

- [Status: Done] **タスク C: `Action→Command` の検証テスト追加**
	- 変換の単一入口（`dm_toolkit.action_to_command`）の動作は `tests/test_execution_architecture.py` 等で担保済み。

- [Status: WIP] **タスク D: CIワークフロー整備**
	- 目的: pytest, mypy, lint を自動実行し、ヘッドレスGUIスタブ検証を含める。
	- 現在、手動での `pytest` 実行は安定しているが、CIオートメーションへの完全移行は継続中。
