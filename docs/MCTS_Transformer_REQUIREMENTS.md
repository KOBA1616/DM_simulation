...
- エンコーディング／アクション変換（C++）
  ...
  - `ai/encoders/ActionEncoder`：Action → 固定長インデックス（`TOTAL_ACTION_SIZE`）
...
1. MCTS 側
   ...
   - 展開時に `IntentGenerator::generate_legal_actions()` を呼び出し、`ActionEncoder::action_to_index()` で policy のインデックスへマップする。
   - ルート展開時に事前確率にディリクレノイズを付与する。

2. Transformer 側（推論）
   - モデルは入力としてトークン列（`TokenConverter::encode_state`）を受け取り、出力として `(policy_logits:[action_dim], value:[1])` を返す。出力 action_dim は `ActionEncoder::TOTAL_ACTION_SIZE` と整合すること。
...
- 再現性: 学習と評価で同じ `TokenConverter` と `ActionEncoder` を使用し、シード管理を行う。

現状のギャップ（要対応）
- ビルド環境: C++ 側で `USE_ONNXRUNTIME` を有効にするには環境でのダウンロード（FetchContent）・リンクが必要。Windows では事前に Visual Studio と適切なツールチェインが必要。
- 入出力整合性: `TokenConverter` のトークン仕様（語彙サイズ・パディング ID 等）と `DuelTransformer` の `vocab_size`、および `ActionEncoder::TOTAL_ACTION_SIZE` が一致していることを明示的に検証する必要あり。
...
4. 整合テスト: MCTS を走らせ、`ActionEncoder` インデックスへのマッピングが正しく行われ、生成された policy が違法行動を受け取らないことを確認する。
...
- Token/Tensor/Action Encoders: `src/ai/encoders/token_converter.cpp`, `src/ai/encoders/tensor_converter.cpp`, `src/ai/encoders/action_encoder.cpp`
