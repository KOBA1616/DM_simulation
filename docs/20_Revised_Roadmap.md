...
### Task 1.1: 環境とビルドの安定化
- [ ] **Python Import Issue**: Windows/MinGW環境等で報告されているDLLロードエラー等の問題を調査・解決し、`import dm_ai_module` が確実に成功する状態にする。
- [ ] **Build Pipeline**: `cmake .. && make` の手順で確実に最新バイナリが生成され、Pythonから参照できる状態を維持する。
...
### Task 3.1: シナリオモードの統合
- [ ] **Scenario Execution**: `ScenarioConfig` を用いて、「特定のコンボ始動盤面」や「詰めろの盤面」からゲームを開始するPythonスクリプトを作成する。

### Task 3.2: データ収集と学習ループの構築
- [ ] **Data Collection Loop**: ランダムAIまたはルールベースAIを用いてシナリオをプレイし、`CardStats` および「勝敗結果」を収集・保存するパイプラインを構築する。
- [ ] **Simple Training**: 収集したデータを用いて、シンプルなMLPモデル（PyTorch）を学習させるスクリプトを作成する。

### Task 3.3: 性能向上の検証 (Verification)
- [ ] **Impact Analysis**: 学習後のモデルが、学習前よりも該当シナリオの勝率（または正解手選択率）において向上していることを数値で証明する。
- [ ] **Loop Detection Verification**: 無限ループを含むシナリオにおいて、ループ検知機能が正しく「引き分け/勝利」を判定できるか確認する。
...
### Task 4.2: 高度な学習機能の導入
- [ ] **PBT (Population Based Training)**: ハイパーパラメータの自動探索を導入。
- [ ] **League Training**: 過去の自分との対戦システムの導入。
- [ ] **POMDP**: 非公開領域推論の導入。
