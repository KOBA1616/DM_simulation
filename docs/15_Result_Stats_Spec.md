...
### 統計データの収束性対策 (Stats Convergence Strategy)
16次元のスタッツが有意な値に収束するまで、学習プロセスを3段階に分けて運用する。

1.  **Mask Phase (マスク期間)**:
    - **動作**: AIへのスタッツ入力（16次元ベクトル）を全て `0.0` にマスクする。
    - **目的**: 不正確な初期スタッツによる学習の混乱（ノイズ）を防ぐ。AIには純粋な盤面情報のみで基礎ルールを学ばせる。
    - **期間**: 最初の 100 epoch 程度。

2.  **Random Baseline Phase (ランダムプレイ収集期間)**:
    - **動作**: バックグラウンドでランダムAI同士の高速対戦を行い、スタッツの初期値（ベースライン）を作成する。
    - **目的**: 極端に弱いカードや強いカードの大まかな傾向を掴む。
    - **適用**: Mask Phase と並行して実施し、ある程度の試行回数（例: 100万回）を超えた時点で次のフェーズへ移行。

3.  **Production Phase (本運用)**:
    - **動作**: 実際に学習中のAIモデルが生成した対戦結果でスタッツを更新し、それをAIの入力として解禁する。
    - **目的**: 「上手いプレイヤー（AI）」による統計データを用い、高度なメタゲーム判断を行わせる。
