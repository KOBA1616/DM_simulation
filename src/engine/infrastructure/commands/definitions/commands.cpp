...
void HistoryCommand::execute(core::GameState &state) {
  state.played_cards_history_this_game[player_id].push_back(
      {card_id, turn_played});
  // Note: Global stats updates from on_card_play are not reversible here and
  // are assumed to be analytical only, or handled separately. Turn stats should
  // be handled by StatCommand if needed, but on_card_play does it too. For now,
  // we only sync the history vector which is part of state.
}
...
void TransitionCommand::execute(core::GameState &state) {
  ...
  auto get_vec = [&](core::Zone z) -> std::vector<core::CardInstance> * {
    ...
    case core::Zone::HAND:
      return &owner.hand;
    case core::Zone::MANA:
      return &owner.mana_zone;
    case core::Zone::BATTLE:
      return &owner.battle_zone;
    case core::Zone::GRAVEYARD:
      return &owner.graveyard;
    case core::Zone::SHIELD:
      return &owner.shield_zone;
    case core::Zone::DECK:
      return &owner.deck;
    case core::Zone::BUFFER:
      return &owner.effect_buffer;
    case core::Zone::STACK:
      return &owner.stack;
    default:
      return nullptr;
    }
  ...
  if (!source_vec || !dest_vec)
    return; // Error

  // Find
  auto it = std::find_if(source_vec->begin(), source_vec->end(),
                         [&](const core::CardInstance &c) {
                           return c.instance_id == card_instance_id;
                         });
  ...
  // Ensure ownership is updated if moving between players (though
  // TransitionCommand is usually intra-player, the owner_id implies the
  // destination owner context). Since owner_id is passed to TransitionCommand,
  // it effectively sets the controller.
  card.owner = owner_id;
  ...
  // --- G-Neo Handling ---
  // Requirement: "G-Neo Creature... when it leaves the field, if there is a
  // card under this creature, instead of leaving, all cards under it are put in
  // the graveyard."

  bool should_replace = false;
  if (from_zone == core::Zone::BATTLE && to_zone != core::Zone::BATTLE &&
      !card.underlying_cards.empty()) {
  ...
  if (should_replace) {
    ...
    for (const auto &under : card.underlying_cards) {
      ...
      if (state.event_dispatcher) {
        ...
        evt.player_id = owner_id;
        evt.context["from_zone"] = static_cast<int>(
            core::Zone::BATTLE); // Effectively from under battle card
  ...
  try {
    ...
    if (lout) {
      lout << "[Transition] MOVED id=" << card_instance_id
           ...
           << " to=" << static_cast<int>(to_zone) << " owner=" << owner_id;
...
void TransitionCommand::invert(core::GameState &state) {
  ...
  core::Player &owner = state.players[owner_id];
  std::vector<core::CardInstance> *source_vec =
      nullptr; // Note: Invert swaps source/dest
  std::vector<core::CardInstance> *dest_vec = nullptr;
  ...
  // --- G-Neo Undo Logic ---
  if (g_neo_activated) {
    // Restore underlying cards from Graveyard to Battle Zone (under the
    // creature). The creature (card_instance_id) is still in from_zone
    // (BATTLE).
    ...
    for (const auto &moved_card : moved_underlying_cards) {
      auto git = std::find_if(grave.begin(), grave.end(),
                              [&](const core::CardInstance &c) {
                                return c.instance_id == moved_card.instance_id;
                              });
      if (git != grave.end()) {
        grave.erase(git);
      }
...
void AddCardCommand::execute(core::GameState &state) {
  if (player_id >= state.players.size())
    return;
...
void AddCardCommand::invert(core::GameState &state) {
  if (player_id >= state.players.size() || added_index < 0)
    return;
  core::Player &p = state.players[player_id];
  std::vector<core::CardInstance> *vec = nullptr;
...
void MutateCommand::execute(core::GameState &state) {
  ...
  case MutationType::REMOVE_KEYWORD: {
    auto it = std::find(card->added_keywords.begin(),
                        card->added_keywords.end(), str_value);
...
void MutateCommand::invert(core::GameState &state) {
  ...
  case MutationType::POWER_MOD:
    ...
    break;
  case MutationType::ADD_KEYWORD: {
    auto it = std::find(card->added_keywords.rbegin(),
                        card->added_keywords.rend(), str_value);
...
void AttachCommand::invert(core::GameState &state) { (void)state; }
...
void FlowCommand::execute(core::GameState &state) {
  ...
  case FlowType::SET_MANA_CHARGED:
    previous_bool_value = state.turn_stats.mana_charged_by_player[player_id];
    state.turn_stats.mana_charged_by_player[player_id] = (new_value != 0);
    // DEBUG: Log the flag change
    try {
      std::ofstream ofs("logs/mana_phase_debug.txt", std::ios::app);
      if (ofs) {
        ofs << "[FlowCommand] SET_MANA_CHARGED from "
            << (previous_bool_value ? "TRUE" : "FALSE") << " to "
            << (state.turn_stats.mana_charged_by_player[player_id] ? "TRUE"
                                                                   : "FALSE")
            << " (new_value=" << new_value << " target_pid=" << (int)player_id
  ...
  case FlowType::CLEANUP_STEP: {
    ...
    for (const auto &m : mods) {
      if (m.turns_remaining == 1)
        removed_modifiers.push_back(m);
    }
    ...
    for (const auto &p : passives) {
      if (p.turns_remaining == 1)
        removed_passives.push_back(p);
    }
    passives.erase(std::remove_if(passives.begin(), passives.end(),
                                  [](core::PassiveEffect &p) {
  ...
  case FlowType::SET_BLOCKING_CREATURE:
    ...
    if (new_value != -1)
      state.current_attack.blocked = true;
    else
...
void FlowCommand::invert(core::GameState &state) {
  ...
  case FlowType::SET_PLAYED_WITHOUT_MANA:
    ...
    break;
  case FlowType::SET_MANA_CHARGED:
    state.turn_stats.mana_charged_by_player[player_id] = previous_bool_value;
    break;
  case FlowType::RESET_TURN_STATS:
...
void DecideCommand::execute(core::GameState &state) {
  ...
  if (state.waiting_for_user_input &&
      state.pending_query.query_id == query_id) {
    state.waiting_for_user_input = false;
  }
}
...
void DeclareReactionCommand::execute(core::GameState &state) {
  ...
  if (!pass) {
    ...
    if (candidate.type == dm::engine::systems::ReactionType::SHIELD_TRIGGER) {
      core::PendingEffect eff(core::EffectType::TRIGGER_ABILITY,
                              candidate.instance_id, candidate.player_id);
      state.pending_effects.push_back(eff);
    } else if (candidate.type ==
               dm::engine::systems::ReactionType::REVOLUTION_CHANGE) {
      core::PendingEffect eff(core::EffectType::TRIGGER_ABILITY,
                              candidate.instance_id, candidate.player_id);
      state.pending_effects.push_back(eff);
    }
...
void ShuffleCommand::invert(core::GameState &state) {
  ...
  if (deck.size() != original_deck_order.size()) {
    // Something went wrong or deck size changed (unlikely within undo stack
    // constraints) Fallback: try to reconstruct as best as possible or do
    // nothing. Ideally this shouldn't happen if undo stack is consistent.
    return;
  }
  ...
  for (int id : original_deck_order) {
    auto it = std::find_if(
        deck.begin(), deck.end(),
        [id](const core::CardInstance &c) { return c.instance_id == id; });
    if (it != deck.end()) {
