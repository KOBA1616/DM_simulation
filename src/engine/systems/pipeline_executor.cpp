...
namespace dm::engine::systems {
    ...
    void log_debug(const std::string& msg) {
        std::ofstream log_file("/tmp/debug_log.txt", std::ios_base::app);
        log_file << msg << std::endl;
        std::cout << "[PIPELINE] " << msg << std::endl; // Also print to stdout
    }

    void PipelineExecutor::execute(const std::vector<Instruction>& instructions, GameState& state,
                                   const std::map<core::CardID, core::CardDefinition>& card_db) {
        log_debug("PipelineExecutor::execute start. Instructions: " + std::to_string(instructions.size()));
        for (const auto& inst : instructions) {
            if (execution_paused) {
                 log_debug("PipelineExecutor paused.");
                 break;
            }
    ...
    void PipelineExecutor::execute_instruction(const Instruction& inst, GameState& state,
        ...
        switch (inst.op) {
            case InstructionOp::GAME_ACTION: {
                ...
                } else if (type == "ATTACH_CARD") { // Added ATTACH_CARD support
                    // Implementation logic for ATTACH_CARD (e.g. evolution source)
                    // Currently relying on RESOLVE_PLAY with evo_source_id for evolution.
                    // If standalone attach needed:
                    // GameLogicSystem::handle_attach_card(*this, state, inst, card_db);
                }
                break;
            }
            ...
            case InstructionOp::WAIT_INPUT: {
                log_debug("  WAIT_INPUT encountered.");
    ...
    void PipelineExecutor::handle_select(const Instruction& inst, GameState& state,
        ...
        for (PlayerID pid : {player_id, static_cast<PlayerID>(1 - player_id)}) {
            ...
            // FIXED: Using string comparison for owner field which is optional string
            if (filter.owner.has_value()) {
                std::string o = filter.owner.value();
                if (o == "PLAYER_SELF" || o == "SELF") {
                    if (pid == player_id) check_player = true;
                } else if (o == "PLAYER_OPPONENT" || o == "OPPONENT") {
                    if (pid != player_id) check_player = true;
                } else {
                    check_player = true; // Default/Both/Invalid
                }
            } else {
                check_player = true;
            }

            if (!check_player) continue;
        ...
        log_debug("Pipeline Select: Found " + std::to_string(valid_targets.size()) + ", Selected " + std::to_string(selection.size()));
    }
    ...
    void PipelineExecutor::handle_move(const Instruction& inst, GameState& state) {
        ...
        log_debug("Pipeline Move: To " + to_zone_str + ", Targets: " + std::to_string(targets.size()) + (is_virtual_target ? " (Virtual)" : ""));
        ...
        if (is_virtual_target) {
            ...
            if (virtual_target_type == "DECK_TOP") {
                ...
                log_debug("  Virtual Deck Top: Available " + std::to_string(available) + ", taking " + std::to_string(count));
        ...
        for (int id : targets) {
             ...
             if (!card_ptr) {
                 PlayerID pid = state.active_player_id;
                 const auto& d = state.players[pid].deck;
                 auto it = std::find_if(d.begin(), d.end(), [id](const CardInstance& c){ return c.instance_id == id; });
                 if (it != d.end()) card_ptr = &(*it);
             }

             if (!card_ptr) {
                 log_debug("  Move Error: Card " + std::to_string(id) + " not found.");
                 continue;
             }
             ...
             if (!found) {
                 log_debug("  Move Error: Card " + std::to_string(id) + " found via pointer but not in any zone of owner " + std::to_string(owner));
                 continue;
             }

             int dest_idx = to_bottom ? 0 : -1;
             log_debug("  Moving " + std::to_string(id) + " from " + std::to_string((int)from_zone) + " to " + std::to_string((int)to_zone));
