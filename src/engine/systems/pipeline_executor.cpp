...
namespace dm::engine::systems {
    ...
    void PipelineExecutor::handle_move(const Instruction& inst, GameState& state) {
        ...
        for (int id : targets) {
             ...
             auto cmd = std::make_shared<TransitionCommand>(id, static_cast<int>(from_zone), static_cast<int>(to_zone), owner, -1);
    ...
    void PipelineExecutor::handle_modify(const Instruction& inst, GameState& state) {
        ...
        for (int id : targets) {
            auto cmd = std::make_shared<MutateCommand>(id, type, val, str_val);
            state.execute_command(cmd);
        }
    }
