                ...
                for (int target_id : targets) {
                    ...
                    if (inst) {
                         ...
                         if (!cmd.from_zone.empty()) {
                             current_zone = parse_zone_string(cmd.from_zone);
                             found = true;
                         } else {
                             PlayerID owner = inst->owner;
                             // Fast check
                             if (dm::engine::ZoneUtils::card_in_zone(state.players[owner].battle_zone, target_id)) { current_zone = Zone::BATTLE; found = true; }
                             else if (dm::engine::ZoneUtils::card_in_zone(state.players[owner].mana_zone, target_id)) { current_zone = Zone::MANA; found = true; }
                             else if (dm::engine::ZoneUtils::card_in_zone(state.players[owner].shield_zone, target_id)) { current_zone = Zone::SHIELD; found = true; }
                             else if (dm::engine::ZoneUtils::card_in_zone(state.players[owner].graveyard, target_id)) { current_zone = Zone::GRAVEYARD; found = true; }
                         }
