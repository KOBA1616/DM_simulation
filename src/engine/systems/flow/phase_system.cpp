...
static void move_card_cmd(GameState &state, std::vector<CardInstance> &from,
                          Zone from_zone, Zone to_zone, PlayerID owner) {
  if (from.empty())
    return;
  int iid = from.back().instance_id;
  auto cmd =
      std::make_unique<TransitionCommand>(iid, from_zone, to_zone, owner, -1);
  ...
  for (auto &player : game_state.players) {
    for (int i = 0; i < 5; ++i) {
      if (player.deck.empty())
        break;
      move_card_cmd(game_state, player.deck, Zone::DECK, Zone::SHIELD,
                    player.id);
    }
    // Draw Hand (5 cards)
    for (int i = 0; i < 5; ++i) {
...
void PhaseSystem::setup_scenario(
    GameState &state, const ScenarioConfig &config,
    const std::map<CardID, CardDefinition> &card_db) {
  // 1. Reset Game State
  state.turn_number = 5;
  ...
  auto populate_owner = [&](const Player &p) {
    auto register_cards = [&](const std::vector<CardInstance> &cards) {
      for (const auto &c : cards) {
        if (c.instance_id >= 0 &&
            c.instance_id < (int)state.card_owner_map.size()) {
...
void PhaseSystem::handle_pass(GameState &state,
                              const std::map<CardID, CardDefinition> &card_db) {
  next_phase(state, card_db);
}

void PhaseSystem::on_start_turn(
    GameState &game_state, const std::map<CardID, CardDefinition> &card_db) {
  Player &active_player = game_state.players[game_state.active_player_id];

  // Reset Turn Stats
  auto cmd_stats =
      std::make_shared<FlowCommand>(FlowCommand::FlowType::RESET_TURN_STATS, 0);
  ...
  for (auto &card : active_player.battle_zone) {
    auto cmd = std::make_shared<MutateCommand>(
        card.instance_id, MutateCommand::MutationType::SET_SUMMONING_SICKNESS,
        0);
  ...
  for (const auto &card : active_player.battle_zone) {
    if (card_db.count(card.card_id)) {
      ...
      if (def.keywords.at_start_of_turn) {
        PendingEffect pe(EffectType::AT_START_OF_TURN, card.instance_id,
                         active_player.id);
...
void PhaseSystem::on_draw_phase(
  ...
  if (!skip_draw) {
    ...
    if (player.deck.empty()) {
      ...
      GameResult loss_result =
          (player.id == 0) ? GameResult::P2_WIN : GameResult::P1_WIN;
...
void PhaseSystem::on_mana_phase(
    GameState &game_state, const std::map<CardID, CardDefinition> &card_db) {
...
void PhaseSystem::on_end_turn(GameState &game_state,
  ...
  {
    auto cmd_src = std::make_shared<FlowCommand>(
        FlowCommand::FlowType::SET_ATTACK_SOURCE, -1);
    game_state.execute_command(std::move(cmd_src));
    auto cmd_tgt = std::make_shared<FlowCommand>(
        FlowCommand::FlowType::SET_ATTACK_TARGET, -1);
    game_state.execute_command(std::move(cmd_tgt));
    auto cmd_plyr = std::make_shared<FlowCommand>(
        FlowCommand::FlowType::SET_ATTACK_PLAYER, -1);
    game_state.execute_command(std::move(cmd_plyr));
    auto cmd_blk = std::make_shared<FlowCommand>(
        FlowCommand::FlowType::SET_BLOCKING_CREATURE, -1);
    game_state.execute_command(std::move(cmd_blk));
  }

  // Cleanup Step
  {
    auto cmd_cleanup =
        std::make_shared<FlowCommand>(FlowCommand::FlowType::CLEANUP_STEP, 0);
      ...
      if (card_db.count(card.card_id)) {
        ...
        if (def.keywords.meta_counter_play) {
          if (game_state.turn_stats.played_without_mana) {
            PendingEffect pe(EffectType::META_COUNTER, card.instance_id,
                             opponent.id);
            TriggerSystem::instance().add_pending_effect(game_state, pe);
          }
        }
    ...
    for (const auto &card : active_player.battle_zone) {
      if (card_db.count(card.card_id)) {
        ...
        if (def.keywords.at_end_of_turn) {
          PendingEffect pe(EffectType::AT_END_OF_TURN, card.instance_id,
                           active_player.id);
          TriggerSystem::instance().add_pending_effect(game_state, pe);
        }
      }
  ...
  // Switch turn
  {
    ...
    if (next_turn != game_state.turn_number) {
      auto cmd_turn = std::make_shared<FlowCommand>(
          FlowCommand::FlowType::TURN_CHANGE, next_turn);
      game_state.execute_command(std::move(cmd_turn));
    }

    auto cmd_active = std::make_shared<FlowCommand>(
        FlowCommand::FlowType::SET_ACTIVE_PLAYER, next_active);
...
void PhaseSystem::next_phase(GameState &game_state,
  ...
  case Phase::BLOCK:
    ...
    if (game_state.current_attack.source_instance_id != -1) {
      ...
      if (blocked) {
        PendingEffect pe(EffectType::RESOLVE_BATTLE,
                         game_state.current_attack.source_instance_id,
                         game_state.active_player_id);
        pe.execution_context["attacker"] =
            game_state.current_attack.source_instance_id;
        pe.execution_context["defender"] =
            game_state.current_attack.blocking_creature_id;
        TriggerSystem::instance().add_pending_effect(game_state, pe);
      } else {
        if (target_id != -1) {
          PendingEffect pe(EffectType::RESOLVE_BATTLE,
          ...
          const Player &opponent =
              game_state.players[1 - game_state.active_player_id];
          if (opponent.shield_zone.empty()) {
            auto win_cmd = std::make_unique<GameResultCommand>(
                game_state.active_player_id == 0 ? GameResult::P1_WIN
                                                 : GameResult::P2_WIN);
            game_state.execute_command(std::move(win_cmd));
          } else {
            PendingEffect pe(EffectType::BREAK_SHIELD,
                             game_state.current_attack.source_instance_id,
                             game_state.active_player_id);
            TriggerSystem::instance().add_pending_effect(game_state, pe);
          }
  ...
  if (next_p != game_state.current_phase) {
    auto cmd = std::make_shared<FlowCommand>(
        FlowCommand::FlowType::PHASE_CHANGE, static_cast<int>(next_p));
    game_state.execute_command(std::move(cmd));
  }
  ...
  if (game_state.current_phase == Phase::START_OF_TURN) {
    ...
    auto cmd_draw = std::make_shared<FlowCommand>(
        FlowCommand::FlowType::PHASE_CHANGE, static_cast<int>(Phase::DRAW));
