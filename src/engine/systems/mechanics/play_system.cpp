...
void PlaySystem::handle_play_card(
    PipelineExecutor &exec, GameState &state, const Instruction &inst,
  ...
  if (origin_int != -1) {
    ...
    if (origin == Zone::DECK)
      origin_str = "DECK";
    else if (origin == Zone::HAND)
      origin_str = "HAND";
    else if (origin == Zone::MANA)
      origin_str = "MANA_ZONE";
    else if (origin == Zone::GRAVEYARD)
      origin_str = "GRAVEYARD";
    else if (origin == Zone::SHIELD)
      origin_str = "SHIELD_ZONE";
  }

  const auto &def = card_db.at(card->card_id);
  if (RestrictionSystem::instance().is_play_forbidden(state, *card, def,
                                                      origin_str, card_db))
    return;
  ...
  if (inst.args.contains("dest_override")) {
    ZoneDestination override_val = static_cast<ZoneDestination>(
        exec.resolve_int(inst.args["dest_override"]));
  ...
  if (dest == Zone::STACK)
    move.args["to"] = "STACK";
  else if (dest == Zone::DECK)
    move.args["to"] = "DECK";
  else if (dest == Zone::HAND)
    move.args["to"] = "HAND";
  else if (dest == Zone::MANA)
    move.args["to"] = "MANA";
  else if (dest == Zone::SHIELD)
    move.args["to"] = "SHIELD";
  else
    move.args["to"] = "BATTLE";
...
void PlaySystem::handle_mana_charge(PipelineExecutor &exec, GameState &state,
  ...
  Instruction move =
      utils::ActionPrimitiveUtils::create_mana_charge_instruction(card_id);
...
void PlaySystem::handle_resolve_play(
    PipelineExecutor &exec, GameState &state, const Instruction &inst,
    const std::map<CardID, CardDefinition> &card_db) {
  ...
  bool is_spell_side =
      exec.resolve_int(inst.args.value("is_spell_side", 0)) != 0;
  const auto &base_def = card_db.at(card->card_id);
  const auto &def =
      (is_spell_side && base_def.spell_side) ? *base_def.spell_side : base_def;
  ...
  if (def.type == CardType::SPELL) {
    ...
    compiled_effects.emplace_back(InstructionOp::MOVE, move_args);
             ...
             def.type == CardType::EVOLUTION_CREATURE) {
...
void PlaySystem::handle_use_ability(
    PipelineExecutor &exec, GameState &state, const Instruction &inst,
    const std::map<CardID, CardDefinition> &card_db) {
  ...
  auto play_cmd = std::make_unique<TransitionCommand>(
  ...
  auto flow_cmd = std::make_shared<FlowCommand>(
      FlowCommand::FlowType::SET_ATTACK_SOURCE, source_id);
  state.execute_command(std::move(flow_cmd));
