...
namespace dm::core {
    ...
    struct CostModifier {
        ...
        int turns_remaining = 0; // 1 = this turn only, >1 = persistent, -1 = indefinite
        int source_instance_id = -1; // To track where it came from (e.g. Cocco Lupia)
    ...
    struct PassiveEffect {
        PassiveType type = PassiveType::POWER_MODIFIER;
        int value = 0; // e.g. +1000 power
        ...
        int source_instance_id = -1; // The source of the effect
        PlayerID controller = 0;
