...
MCTS::MCTS(const std::map<CardID, CardDefinition> &card_db, float c_puct,
    ...
    : card_db_(std::make_shared<std::map<CardID, CardDefinition>>(card_db)),
      c_puct_(c_puct), dirichlet_alpha_(dirichlet_alpha),
      dirichlet_epsilon_(dirichlet_epsilon), batch_size_(batch_size),
      alpha_(alpha) {}

MCTS::MCTS(std::shared_ptr<const std::map<CardID, CardDefinition>> card_db,
           float c_puct, float dirichlet_alpha, float dirichlet_epsilon,
           int batch_size, float alpha)
    : card_db_(card_db), c_puct_(c_puct), dirichlet_alpha_(dirichlet_alpha),
      dirichlet_epsilon_(dirichlet_epsilon), batch_size_(batch_size),
      alpha_(alpha) {}

void MCTS::set_pimc_generator(
    std::shared_ptr<dm::ai::inference::PimcGenerator> pimc) {
  pimc_generator_ = pimc;
}

std::vector<float> MCTS::search(const GameState &root_state, int simulations,
                                BatchEvaluatorCallback evaluator,
                                bool add_noise, float temperature) {
  ...
  if (pimc_generator_) {
    ...
    if (!opp.hand.empty() && opp.hand[0].card_id == 0)
      has_hidden_info = true;
    else if (!opp.shield_zone.empty() && opp.shield_zone[0].card_id == 0)
      has_hidden_info = true;
  }

  if (has_hidden_info) {
    // PIMC Root Parallelization Logic
    std::vector<float> aggregated_policy(CommandEncoder::TOTAL_COMMAND_SIZE,
                                         0.0f);
    ...
    for (int i = 0; i < samples; ++i) {
      // 1. Determinize
      GameState det_state = pimc_generator_->generate_determinized_state(
          root_state, root_state.active_player_id,
          i); // Use loop index as seed offset

      // 2. Run MCTS on this determinized state
      // Note: This recursively calls search, but now on a state without hidden
      // info. However, calling search() overwrites last_root_, which is fine as
      // we only care about the policy return. WARNING: We must ensure the
      // recursive call doesn't trigger PIMC loop again. Since det_state has
      // filled IDs, has_hidden_info will be false next time.

      // Disable PIMC generator temporarily for recursive call?
      // Or rely on the check. The check verifies ID=0. det_state has IDs
      // filled.

      std::vector<float> pol = this->search(det_state, sims_per_sample,
                                            evaluator, add_noise, temperature);
  ...
  dm::engine::flow::PhaseSystem::instance().fast_forward(root_gs, *card_db_);

  // Explicitly free the old tree before allocating the new one to reduce peak
  // memory usage
  ...
  if (dm::ai::inference::NativeInferenceManager::instance().has_model()) {
    ...
    if (!evaluator)
      throw std::runtime_error(
          "No evaluator provided and no native model available");
  ...
  while (simulations_finished < simulations) {
    ...
    int current_batch_limit =
        std::min(batch_size_, simulations - simulations_finished);
    ...
    for (int i = 0; i < current_batch_limit; ++i) {
      ...
      GameResult result;
      bool is_over = dm::engine::flow::PhaseSystem::instance().check_game_over(
          leaf->state, result);
      ...
      if (is_over) {
        ...
        if (result == GameResult::DRAW)
          value = 0.0f;
        else if (result == GameResult::P1_WIN)
          value = (current_player == 0) ? 1.0f : -1.0f;
        ...
        batch_states.push_back(
            std::make_shared<GameState>(leaf->state.clone()));
      }
    }

    if (batch_nodes.empty()) {
      if (simulations_finished >= simulations)
        break;
      continue;
    }
    ...
    for (size_t i = 0; i < batch_nodes.size(); ++i) {
      ...
      if (node->parent != nullptr) {
        float dense = node->state.calculate_board_advantage(
            node->state.active_player_id, *card_db_);
  ...
  if (temperature < 1e-3f) {
    ...
    if (best_child) {
      int idx =
          CommandEncoder::command_to_index(best_child->action_from_parent);
    ...
    for (const auto &child : root->children) {
      float v =
          std::pow(static_cast<float>(child->visit_count), 1.0f / temperature);
      visits_pow.push_back(v);
      sum_visits_pow += v;
    }

    for (size_t i = 0; i < root->children.size(); ++i) {
      int idx = CommandEncoder::command_to_index(
          root->children[i]->action_from_parent);
      if (idx >= 0 && idx < (int)policy.size()) {
...
void MCTS::expand_node(MCTSNode *node,
  ...
  auto actions =
      IntentGenerator::generate_legal_commands(node->state, *card_db_);
  ...
  for (size_t i = 0; i < actions.size(); ++i) {
    ...
    // NOTE: Do NOT call next_phase here! resolve_command_oneshot already
    // handles phase transitions internally. Only fast_forward to reach
    // next decision point for tree search.
    dm::engine::flow::PhaseSystem::instance().fast_forward(next_state,
                                                           *card_db_);

    auto child = std::make_unique<MCTSNode>(
        std::move(next_state)); // next_state is moved
...
MCTSNode *MCTS::select_leaf(MCTSNode *node) {
  while (node->is_expanded()) {
    ...
    for (const auto &child : node->children) {
      ...
      float u = c_puct_ * child->prior *
                std::sqrt(static_cast<float>(node->visit_count +
                                             node->virtual_loss)) /
...
void MCTS::backpropagate(MCTSNode *node, float value) {
  while (node) {
    ...
    if (node->parent &&
        node->parent->state.active_player_id != node->state.active_player_id) {
      value = -value;
    }
...
void MCTS::add_exploration_noise(MCTSNode *node) {
  ...
  for (size_t i = 0; i < node->children.size(); ++i) {
    node->children[i]->prior =
        node->children[i]->prior * (1 - dirichlet_epsilon_) +
        (noise[i] / sum) * dirichlet_epsilon_;
  }
}
