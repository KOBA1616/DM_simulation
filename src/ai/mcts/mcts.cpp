    ...
    std::vector<float> MCTS::search(const GameState& root_state, int simulations, BatchEvaluatorCallback evaluator, bool add_noise, float temperature) {
        ...
        while (simulations_finished < simulations) {
            ...
            for (int i = 0; i < current_batch_limit; ++i) {
                ...
                GameResult result;
                bool is_over = dm::engine::flow::PhaseSystem::instance().check_game_over(leaf->state, result);
    ...
    void MCTS::expand_node(MCTSNode* node, const std::vector<float>& policy_logits) {
        ...
        for (size_t i = 0; i < actions.size(); ++i) {
            ...
            if (action.type == CommandType::PASS || action.type == CommandType::MANA_CHARGE) {
                dm::engine::flow::PhaseSystem::instance().next_phase(next_state, *card_db_);
            }
            dm::engine::flow::PhaseSystem::instance().fast_forward(next_state, *card_db_);
