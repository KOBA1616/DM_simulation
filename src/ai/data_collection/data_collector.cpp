...
DataCollector::DataCollector(
    std::shared_ptr<const std::map<dm::core::CardID, dm::core::CardDefinition>>
        card_db)
    : card_db_(card_db) {}

DataCollector::DataCollector(
    const std::map<dm::core::CardID, dm::core::CardDefinition> &card_db)
    : card_db_(std::make_shared<
...
CollectedBatch
DataCollector::collect_data_batch_heuristic(int episodes, bool collect_tokens,
                                            bool collect_tensors) {
  ...
  for (int i = 0; i < episodes; ++i) {
    ...
    auto setup_deck = [&](dm::core::Player &p,
                          const std::vector<dm::core::CardID> &deck_list) {
      p.deck.clear();
    ...
    while (game.state.winner == dm::core::GameResult::NONE &&
           step < max_steps) {
      step++;
      int active_player = game.state.active_player_id;

      dm::engine::IntentGenerator action_gen;
      auto legal_actions = dm::engine::IntentGenerator::generate_legal_commands(
          game.state, *card_db_);

      // If no actions, transition phase
      if (legal_actions.empty()) {
        dm::engine::flow::PhaseSystem::instance().next_phase(game.state,
                                                             *card_db_);

        dm::core::GameResult res;
        if (dm::engine::flow::PhaseSystem::instance().check_game_over(
                game.state, res)) {
      ...
      if (collect_tokens) {
        ...
        std::vector<long> state_seq = TensorConverter::convert_to_sequence(
            game.state, active_player, *card_db_);
        game_tokens.push_back(state_seq);
      }

      if (collect_tensors) {
        std::vector<float> tensor = TensorConverter::convert_to_tensor(
            game.state, active_player, *card_db_);
      ...
      // Apply action
      GameLogicSystem::resolve_command_oneshot(game.state, chosen_action,
                                               *card_db_);
      ...
      dm::core::GameResult res;
      if (dm::engine::flow::PhaseSystem::instance().check_game_over(game.state,
                                                                    res)) {
        game.state.winner = res;
      }
    }
    ...
    for (size_t k = 0; k < game_policies.size(); ++k) {
      if (collect_tokens)
        batch.token_states.push_back(game_tokens[k]);
      if (collect_tensors)
        batch.tensor_states.push_back(game_tensors[k]);
