...
namespace dm::ai {
    ...
    GameResultInfo ScenarioExecutor::run_scenario(const ScenarioConfig& config, int max_steps) {
        ...
        while (steps < max_steps) {
            bool game_over = dm::engine::systems::PhaseSystem::instance().check_game_over(instance.state, result);
            if (game_over) {
                break;
            }
            ...
            if (legal_actions.empty()) {
                dm::engine::systems::PhaseSystem::instance().next_phase(instance.state, *card_db);
                continue;
            }
