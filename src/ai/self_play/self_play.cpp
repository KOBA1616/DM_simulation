...
SelfPlay::SelfPlay(const std::map<CardID, CardDefinition> &card_db,
                   int mcts_simulations, int batch_size)
...
SelfPlay::SelfPlay(
    ...
    int mcts_simulations, int batch_size)
    : card_db_(card_db), mcts_simulations_(mcts_simulations),
      batch_size_(batch_size) {}

void SelfPlay::set_pimc_generator(
    std::shared_ptr<dm::ai::inference::PimcGenerator> pimc_generator) {
  pimc_generator_ = pimc_generator;
}

GameResultInfo SelfPlay::play_game(const GameState &initial_state,
                                   BatchEvaluatorCallback evaluator,
                                   float temperature, bool add_noise,
                                   float alpha, bool collect_data) {
  GameResultInfo info;
  ...
  while (true) {
    ...
    if (dm::engine::flow::PhaseSystem::instance().check_game_over(state,
                                                                  result)) {
      info.result = result;
    ...
    // If PIMC is NOT enabled, we must determinize the state (hidden information
    // handling) before passing it to MCTS, assuming perfect information search
    // (Open Loop). If PIMC IS enabled, MCTS handles the
    // determinization/sampling internally via PimcGenerator.
    if (!pimc_generator_) {
      Determinizer::determinize(search_state, state.active_player_id);
    }

    std::vector<float> policy = mcts.search(search_state, mcts_simulations_,
                                            evaluator, add_noise, temperature);

    if (collect_data) {
      // info.states.push_back(state); // Copy -> Deleted
      info.states.push_back(
          std::make_shared<dm::core::GameState>(state.clone()));
      info.policies.push_back(policy);
      info.active_players.push_back(state.active_player_id);
    }

    auto legal_actions =
        IntentGenerator::generate_legal_commands(state, *card_db_);
    if (legal_actions.empty()) {
      dm::engine::flow::PhaseSystem::instance().next_phase(state, *card_db_);
      continue;
    }
    ...
    // handles phase transitions internally via dispatch_command:
    //   - PASS → handle_pass → next_phase
    //   - MANA_CHARGE → handle_mana_charge + fast_forward
  }
