...
Commands:
  status           - show basic game/session state
  hand [pid]       - list hand cards for player id (default 0)
  deck [pid]       - show deck size for player
  legal            - list legal commands for active player
  play <instance>  - attempt to play an instance id from hand
  play-first       - attempt to play first playable hand instance
  step             - advance one phase step
  run <n>          - run up to n steps or until game over
  auto <n>         - auto loop: try play-first then step, n iterations
  dump             - dump full players state (hand/deck counts)
  exit / quit      - quit

Use `--auto` to run non-interactively and exit.
"""
import argparse
import sys
import time
...
def main():
    ...
    parser.add_argument('--p0-human', action='store_true')
    parser.add_argument('--p1-human', action='store_true')
    parser.add_argument('--log-level', type=str, default='INFO', help='logging level (DEBUG, INFO, WARNING)')
    parser.add_argument('--dump-json', type=str, default=None, help='dump minimal game state to this JSON file on exit')
    parser.add_argument('--auto', type=int, default=None, help='run auto loop N iterations and exit')
    ...
    logger = logging.getLogger('headless_mainwindow')
