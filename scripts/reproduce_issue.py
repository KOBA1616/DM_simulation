...
def run():
    ...
    if TORCH_AVAILABLE:
        ...
        print("Torch not available, using None for model.")

    # 3. Instantiate MCTS (WITHOUT state_converter)
    # This should trigger the RuntimeError (Deprecation)
    print("Initializing MCTS (Default Configuration)...")
    try:
        mcts = MCTS(network=model, card_db=card_db, simulations=2)
        print("CRITICAL: MCTS initialized successfully (Unexpected)!")
    except RuntimeError as e:
        print("\nSUCCESS: Caught expected deprecation error!")
        print(f"Exception Type: {type(e).__name__}")
        print(f"Exception Message: {e}")
        return

    # 4. Create State (Unreachable)
    state = GameState()
