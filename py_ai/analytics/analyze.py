...
class Analytics:
    ...
    def generate_heatmap(self, output_path="data/analytics/heatmap.png"):
        ...
        # Y-ticks (Card IDs) - might be too many, so sample or just show indices
        if len(sorted_card_ids) < 50:
            plt.yticks(range(len(sorted_card_ids)), sorted_card_ids)
        else:
            plt.yticks(range(0, len(sorted_card_ids), 5), sorted_card_ids[::5])
    ...
    def generate_report(self, output_path="data/analytics/report.html"):
        ...
        html.append('<img src="heatmap.png" alt="Card Usage Heatmap" style="max-width:100%">')
    ...
    if not glob.glob("data/generations/gen_*.json"):
        ...
        for i in range(5):
            data = {
                "generation": i,
                "decks": [
                    [np.random.randint(1, 20) for _ in range(40)] for _ in range(10)
                ]
            }
