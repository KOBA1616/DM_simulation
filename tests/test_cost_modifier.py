...
def test_cost_modifier_static_reduction():
    ...
    # Updated: ActionGenerator now produces DECLARE_PLAY for cards in hand
    can_play = any(a.type == dm_ai_module.ActionType.DECLARE_PLAY and a.card_id == 100 for a in actions)
...
def test_cost_modifier_filtered():
    ...
    actions = dm_ai_module.ActionGenerator.generate_legal_actions(state, card_db)
    can_play_dragon = any(a.type == dm_ai_module.ActionType.DECLARE_PLAY and a.card_id == 100 for a in actions)
    ...
    actions = dm_ai_module.ActionGenerator.generate_legal_actions(state, card_db)
    can_play_human = any(a.type == dm_ai_module.ActionType.DECLARE_PLAY and a.card_id == 101 for a in actions)
    assert not can_play_human, "Human should NOT get cost reduction (only have 3 mana for cost 5)"
