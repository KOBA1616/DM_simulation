...
def test_shield_burn_with_amount():
    ...
    out = _call_format_command('SHIELD_BURN', action, False, 3, 0, '', '', input_key='', input_usage='', sample=[])
    assert '相手のシールドを3つ選び、墓地に置く。' == out


def test_shield_burn_default_amount():
    action = {'type': 'SHIELD_BURN'}
    out = _call_format_command('SHIELD_BURN', action, False, 0, 0, '', '', input_key='', input_usage='', sample=[])
