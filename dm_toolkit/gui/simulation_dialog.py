...
from dm_toolkit.training.scenario_definitions import SCENARIOS # Needed for Combo Box population
...
class SimulationWorker(QThread):
    ...
    def run(self):
        ...
        self.runner = BatchSimulationRunner(
            self.card_db, self.scenario_name, self.episodes,
            self.threads, self.sims, self.evaluator_type, self.model_path
        )
        ...
        try:
            win_rate, summary = self.runner.run(on_progress)
            self.finished_signal.emit(win_rate, summary)
        except Exception as e:
            self.finished_signal.emit(0.0, f"Error: {e}")
