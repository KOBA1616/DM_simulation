...
class CardTextGenerator:
    ...
    def _format_effect(cls, effect: Dict[str, Any], ctx: TextGenerationContext) -> str:
        ...
        for command in commands:
            ...
            if isinstance(command_for_text, dict):
                ...
                if out_key:
                    inferred_label = InputLinkFormatter.infer_output_label(command_for_text)
                    if inferred_label:
    ...
    def _format_game_action_command(cls, atype: str, action: Dict[str, Any], val1: int, val2: int, target_str: str, unit: str, input_key: str, input_usage: str, ctx: TextGenerationContext) -> str:
        ...
        def _handle_draw_card():
            ...
            if has_input_key:
                linked_count = InputLinkFormatter.format_count_token(action, "その同じ枚数")
                if up_to:
        ...
        if handler:
            ...
            return "\n".join(lines)
             ...
             if stat_name:
                 ...
                 if input_key:
                     usage_label = InputLinkFormatter.format_usage_label(input_usage)
                     if usage_label:
                         base += f"（{usage_label}）"
                 return base
    ...
    def _format_zone_move_command(cls, atype: str, action: Dict[str, Any], is_spell: bool, val1: int, target_str: str) -> str:
        ...
        if atype == "TRANSITION":
                ...
                if to_z == "GRAVEYARD":
                    ...
                    if input_key:
                        ...
                        if normalized_from == "HAND":
                            ...
                            linked_count = InputLinkFormatter.format_count_token(action, "その同じ数")
                            owner = ""
                            if scope in ["PLAYER_SELF", "SELF"]:
                                owner = "自分の"
            ...
            if input_key:
                ...
                link_suffix = InputLinkFormatter.format_context_suffix(action)
                linked_target = "そのカード"
    ...
    def _format_action(cls, action: Dict[str, Any], ctx: TextGenerationContext) -> str:
        ...
        if input_key:
            ...
            usage_label_suffix = ""
            if input_usage:
                label = InputLinkFormatter.format_usage_label(input_usage)
                if label:
        ...
        if atype == "GRANT_KEYWORD" or atype == "ADD_KEYWORD":
            ...
            if "SPELL" in types or not types:
                ...
                if is_input_linked(max_cost_def, usage="MAX_COST"):
                    source_token = InputLinkFormatter.format_count_token(action, "その数")
                    source_token = InputLinkFormatter.normalize_count_label(source_token)
