...
class CardTextGenerator:
    ...
    def generate_body_text(cls, data: Dict[str, Any], sample: List[Any] = None) -> str:
        ...
        for cr in cost_reductions:
            text = ActionTextGenerator.format_cost_reduction(cr)
            if text:
                lines.append(f"■ {text}")
        ...
        for r in reactions:
            text = ActionTextGenerator.format_reaction(r)
            if text:
                lines.append(f"■ {text}")
        ...
        for effect in effects:
            if _is_special_only_effect(effect):
                continue
            # Check if this card has mega_last_burst keyword
            has_mega_last_burst = data.get("keywords", {}).get("mega_last_burst", False)
            text = ActionTextGenerator.format_effect(effect, is_spell, sample=sample, card_mega_last_burst=has_mega_last_burst)
            if text:
                lines.append(f"■ {text}")
        ...
        for static_ability in static_abilities:
            if static_ability and isinstance(static_ability, dict):
                text = ActionTextGenerator.format_effect(static_ability, is_spell, sample=sample)
                if text:
                    lines.append(f"■ {text}")
        ...
        if metamorphs:
            ...
            for effect in metamorphs:
                text = ActionTextGenerator.format_effect(effect, is_spell, sample=sample)
                if text:
                    lines.append(f"■ {text}")
