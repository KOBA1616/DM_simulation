...
from dm_toolkit.gui.editor.forms.base_form import to_dict, get_attr
...
class VariableLinkWidget(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self._produces_output = True  # 再発防止: produces_output=False 時は output_key_edit を常時非表示にする。
        self.setup_ui()
    ...
    def setup_ui(self):
        ...
        safe_connect(self.output_key_edit, "textEdited", self._on_output_edited)

        # Connect signals
        safe_connect(self.input_key_combo, "currentIndexChanged", self.on_combo_changed)
        safe_connect(self.input_usage_combo, "currentIndexChanged", self.linkChanged.emit)
        safe_connect(self.output_key_edit, "textChanged", self.linkChanged.emit)
    ...
    def set_current_item(self, item):
        ...
        if action_data:
            # Handle both dict and CommandModel
            if isinstance(action_data, CommandModel):
                act_type = action_data.type
            else:
    ...
    def set_data(self, data):
        ...
        input_key = data.get('input_value_key', '') or data.get('input_var', '')
    ...
    def get_value(self):
        """Interface-based API to return dict of current values."""
        data = {}
        self.get_data(data)
        return data
    ...
    def _update_output_visibility(self):
        ...
        # 再発防止: produces_output=False のコマンドでは常時非表示。
        if not self._produces_output:
            self.output_key_label.setVisible(False)
            self.output_key_edit.setVisible(False)
            return
    ...
    def set_output_hint(self, produces_output):
        ...
        """
        # 再発防止: produces_output=False のコマンドにも output_value_key は属るが
        # UI には表示しない。ユーザーに命名を意識させず自動生成するため。
        self._produces_output = bool(produces_output)
        if produces_output:
            ...
            # 非表示固定: produces_output=False 時はキーの有無に関わらず output_key を隠す
            self.output_key_label.setVisible(False)
            self.output_key_edit.setVisible(False)
