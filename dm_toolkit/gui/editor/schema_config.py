...
def register_all_schemas():
    ...
    # STAT
    stat_keys = list(CardTextResources.STAT_KEY_MAP.keys())
    register_schema(CommandSchema("STAT", [
        FieldSchema("str_param", tr("Stat Key"), FieldType.SELECT, options=stat_keys),
        FieldSchema("amount", tr("Value to Add"), FieldType.INT, default=0),
        f_links_out
    ]))
