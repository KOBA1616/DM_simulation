...
class DrawCardFormatter(CommandFormatterBase):
    ...
    def update_metadata(cls, command: Dict[str, Any], ctx: TextGenerationContext) -> None:
        ctx.metadata[SemanticMetadataFlags.DRAWS.value] = True
    ...
    def format(cls, command: Dict[str, Any], ctx: TextGenerationContext) -> str:
        ...
        if up_to:
            formatted_qty = TextUtils.format_up_to(val1, "枚", up_to)
            # Remove "まで" since template replace might get wonky, or just overwrite it entirely
            template = f"{formatted_qty}引く。"
...
class DiscardFormatter(CommandFormatterBase):
    @classmethod
    def update_metadata(cls, command: Dict[str, Any], ctx: TextGenerationContext) -> None:
        ctx.metadata[SemanticMetadataFlags.DISCARDS.value] = True
    ...
    def format(cls, command: Dict[str, Any], ctx: TextGenerationContext) -> str:
        ...
        qty_text = TextUtils.format_up_to(cnt, unit, up_to)

        # In discard formatting, standard usage omits the trailing 'まで' for aesthetics sometimes,
        # but to keep it unified: "最大X枚まで捨てる" or "X枚捨てる"
        # We can just use the unified string directly
        text = f"{tgt}を{qty_text}捨てる。"
