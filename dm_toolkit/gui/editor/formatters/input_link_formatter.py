...
from dm_toolkit.gui.editor.resources.text_resources import CardTextResources

class InputLinkFormatter:
    """Formatter for handling input-linked command variables and labels."""

    @classmethod
    def format_usage_label(cls, usage: Any) -> str:
    ...
    @classmethod
    def format_source_label(cls, action: Dict[str, Any]) -> str:
    ...
    @classmethod
    def normalize_count_label(cls, label: str) -> str:
    ...
    @classmethod
    def format_count_token(cls, action: Dict[str, Any], fallback: str) -> str:
        """Return count token for linked-input text. Prefer semantic labels over generic wording."""
        label = cls.format_source_label(action)
        if not label:
            return fallback
    ...
    @classmethod
    def infer_output_label(cls, command: Dict[str, Any]) -> str:
    ...
    @classmethod
    def format_context_suffix(cls, action: Dict[str, Any]) -> str:
        """Format input-link metadata used in card preview text."""
        source_label = cls.format_source_label(action)
        usage_raw = action.get("input_value_usage") or action.get("input_usage")
        usage_label = cls.format_usage_label(usage_raw) if usage_raw else ""
