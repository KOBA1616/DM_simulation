...
class CardDataManager:
    ...
    def _ensure_item(self, index_or_item) -> IEditorItem:
        if isinstance(index_or_item, IEditorItem):
            return index_or_item
        return self.model.get_item(index_or_item)
    ...
    def add_child_item(self, parent_index, item_type, data, label):
        ...
        return self.serializer.add_child_item(self.model, parent_item, item_type, data, label)
        ...
        if type_ in ["EFFECT", "OPTION", "CMD_BRANCH_TRUE", "CMD_BRANCH_FALSE"]:
            target_item = item
             ...
             if cmd_type in ["IF", "IF_ELSE", "ELSE"]:
                 ...
                 if not true_branch:
                    true_branch = self.serializer.add_child_item(self.model, item, "CMD_BRANCH_TRUE", None, tr("If True"))

                 # For IF_ELSE, ensure False branch exists so user sees it
                 if cmd_type == "IF_ELSE" and not false_branch:
                    self.serializer.add_child_item(self.model, item, "CMD_BRANCH_FALSE", None, tr("If False"))
                ...
                if not target_item:
                    ...
                    # If item.parent() is None, it might be a child of root or root itself?
                    # IEditorItem parent() returns Optional[IEditorItem].
                    pass
    ...
    def collect_conversion_preview(self, root_handle=None):
        ...
        if root_handle is None:
             ...
             root_item = self._ensure_item(root_handle)
    ...
    def add_command_branches(self, item_handle):
        item = self._ensure_item(item_handle)
        if not item: return

        # This logic was embedded in LogicTreeWidget.generate_branches_for_current
        # but LogicTreeWidget called add_command_branches(item).
        # Wait, I see "self.data_manager.add_command_branches(item)" in previous LogicTreeWidget read.
        # But I don't see `add_command_branches` in `dm_toolkit/gui/editor/data_manager.py` previously read!
        # Ah, maybe I missed it or it was added dynamically?
        # Actually in LogicTreeWidget it says:
        # if item_type == "COMMAND":
        #    self.data_manager.add_command_branches(item)
        # So I should implement it here if it's missing.
        ...
        if cmd_type in ["IF", "IF_ELSE", "ELSE"]:
             # Similar logic to add_command_contextual for creating branches
             ...
             for i in range(item.row_count()):
                 child = item.child(i)
                 role = child.data(ROLE_TYPE)
                 if role == "CMD_BRANCH_TRUE": true_branch = child
                 elif role == "CMD_BRANCH_FALSE": false_branch = child

             if not true_branch:
                 self.serializer.add_child_item(self.model, item, "CMD_BRANCH_TRUE", None, tr("If True"))
             if cmd_type == "IF_ELSE" and not false_branch:
                 self.serializer.add_child_item(self.model, item, "CMD_BRANCH_FALSE", None, tr("If False"))

    def update_effect_type(self, item_handle, target_type):
        item = self._ensure_item(item_handle)
        if not item: return

        # Updates the item's visual state (Label) and Type to match the new effect type.
        # This is used by LogicTreeWidget.move_effect_item

        # Only support Triggered Ability <-> Static Ability logic if needed
        # LogicTreeWidget seems to use it.
        pass
