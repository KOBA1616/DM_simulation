...
def migrate_cost_reductions(card_data: Dict[str, Any]) -> int:
    """Ensure `cost_reductions` entries have `id` fields.

    Returns the number of cost_reduction entries modified (i.e., ids assigned).
    Uses the editor-side utility `generate_missing_ids` when available; otherwise
    assigns simple autogenerated ids.
    """
    if not isinstance(card_data, dict):
        return 0

    crs = card_data.get('cost_reductions')
    ...
    try:
        ...
        # Record before count of ids
        before = sum(1 for cr in crs if isinstance(cr, dict) and cr.get('id'))
        generate_missing_ids(crs)
        after = sum(1 for cr in crs if isinstance(cr, dict) and cr.get('id'))
        modified = max(0, after - before)
        return modified
    except Exception:
        # Fallback: assign auto ids for missing entries
        for cr in crs:
            if isinstance(cr, dict) and not cr.get('id'):
                cr['id'] = f"migrated_{uuid.uuid4().hex[:8]}"
                modified += 1
        return modified
