...
class ActionTextGenerator:
    """
    Handles generation of Japanese rule text for individual actions and commands.
    Extracted from CardTextGenerator to separate concerns.
    """
    ...
    def format_effect(cls, effect: Dict[str, Any], is_spell: bool = False, sample: List[Any] = None, card_mega_last_burst: bool = False) -> str:
        ...
        for command in commands:
            ...
            action_texts.append(cls.format_command(command, is_spell, sample=sample, card_mega_last_burst=card_mega_last_burst))
    ...
    def format_command(cls, command: Dict[str, Any], is_spell: bool = False, sample: List[Any] = None, card_mega_last_burst: bool = False) -> str:
        ...
        for k in ["options", "flags", "look_count", "add_count", "rest_zone", "max_cost", "token_id", "play_flags", "select_count", "if_true", "if_false", "condition"]:
            if k in command_copy:
                action_proxy[k] = command_copy.get(k)
    ...
    @classmethod
    def format_reaction(cls, reaction: Dict[str, Any]) -> str:
        return cls._format_reaction(reaction)

    @classmethod
    def format_cost_reduction(cls, cr: Dict[str, Any]) -> str:
        return cls._format_cost_reduction(cr)
    ...
    def _apply_trigger_scope(cls, trigger_text: str, scope: str, trigger_type: str, trigger_filter: Dict[str, Any] = None) -> str:
        ...
        def _compose_subject_from_filter(default_type: str) -> str:
            ...
            types = f.get("types", [])
            zones = f.get("zones", [])
            min_cost = f.get("min_cost", 0) or 0
            max_cost = f.get("max_cost", 999) if f.get("max_cost") is not None else 999
            exact_cost = f.get("exact_cost")
            cost_ref = f.get("cost_ref")
            min_power = f.get("min_power", 0) or 0
            max_power = f.get("max_power", 999999) if f.get("max_power") is not None else 999999
            ...
            if zones and "BATTLE_ZONE" not in zones:
                ...
                for z in zones:
                    ...
                    if zone_text: zone_names.append(zone_text)
                if zone_names: adjs.append("/".join(zone_names))
            ...
            if cost_ref:
                ...
                if isinstance(min_cost, dict) and min_cost.get("input_value_usage") == "MIN_COST":
                    ...
                    if min_cost > 0 and max_cost < 999: adjs.append(f"コスト{min_cost}～{max_cost}")
                    elif min_cost > 0: adjs.append(f"コスト{min_cost}以上")
                    elif max_cost < 999: adjs.append(f"コスト{max_cost}以下")
            ...
            if power_max_ref:
                ...
                if min_power > 0 and max_power < 999999: adjs.append(f"パワー{min_power}～{max_power}")
                elif min_power > 0: adjs.append(f"パワー{min_power}以上")
                elif max_power < 999999: adjs.append(f"パワー{max_power}以下")
            ...
            if flags:
                for flag in flags:
                    if flag == "BLOCKER":
                        if "ブロッカー" not in adjs: adjs.append("ブロッカー")
    ...
    def _format_logic_command(cls, atype: str, action: Dict[str, Any], is_spell: bool, sample: List[Any], card_mega_last_burst: bool) -> str:
        ...
        if isinstance(cond_detail, dict):
            ...
            if cond_type == "OPPONENT_DRAW_COUNT":
                ...
                op_text = ""
                if op == ">=": op_text = f"{val}{unit}以上"
                elif op == "<=": op_text = f"{val}{unit}以下"
                elif op == "=" or op == "==": op_text = f"{val}{unit}"
                elif op == ">": op_text = f"{val}{unit}より多い"
                elif op == "<": op_text = f"{val}{unit}より少ない"
                ...
                if op == ">=":
                    try:
                        ...
                        op_text = f"{val}以上"
                elif op == "<=": op_text = f"{val}以下"
                elif op == "=" or op == "==": op_text = f"{val}"
                elif op == ">": op_text = f"{val}より多い"
                elif op == "<": op_text = f"{val}より少ない"
        ...
        if atype == "IF":
             ...
             for cmd in if_true_cmds:
                 if isinstance(cmd, dict):
                     cmd_text = cls.format_command(cmd, is_spell, sample, card_mega_last_burst)
                     if cmd_text: if_true_texts.append(cmd_text)
            ...
            for cmd in if_false_cmds:
                if isinstance(cmd, dict):
                    cmd_text = cls.format_command(cmd, is_spell, sample, card_mega_last_burst)
                    if cmd_text: if_false_texts.append(cmd_text)
    ...
    def _format_special_effect_command(cls, atype: str, action: Dict[str, Any], is_spell: bool, val1: int, target_str: str, unit: str) -> str:
        if atype == "MEKRAID":
            ...
            if amt is None: amt = action.get('value1')

            if str_param == "COST":
                amt_val = amt if isinstance(amt, int) else 0
                if is_target_linked: select_phrase = ""
                elif isinstance(amt, int) and amt > 0: select_phrase = f"{target_str}を{amt}{unit}は、"
                else: select_phrase = f"{target_str}を選び、"
            ...
            keyword = CardTextResources.get_keyword_text(str_val)
            if action.get("explicit_self"): target_str = "このカード"
            ...
            return cls._format_keyword_grant_text(target_str, str_val, keyword, duration_text, amt, skip_selection=is_target_linked)
             ...
             if mkind == "TAP": template = "{target}を{amount}{unit}選び、タップする。"
             elif mkind == "UNTAP": template = "{target}を{amount}{unit}選び、アンタップする。"
    ...
    def _format_game_action_command(cls, atype: str, action: Dict[str, Any], is_spell: bool, val1: int, val2: int, target_str: str, unit: str, input_key: str, input_usage: str, sample: List[Any]) -> str:
        ...
        if atype == "SEARCH_DECK":
            ...
            if dest_zone == "HAND": action_phrase = "手札に加える"
            elif dest_zone == "MANA_ZONE": action_phrase = "マナゾーンに置く"
            elif dest_zone == "GRAVEYARD": action_phrase = "墓地に置く"
            else: action_phrase = f"{zone_str}に置く"
            ...
            return template
             ...
             rest_text = ""
             if rest_zone == "DECK_BOTTOM": rest_text = "残りを好きな順序で山札の下に置く。"
             elif rest_zone == "GRAVEYARD": rest_text = "残りを墓地に置く。"
             else: rest_text = f"残りを{tr(rest_zone)}に置く。"

             filter_text = ""
             if target_str != "カード": filter_text = f"{target_str}を"
             ...
             if not action.get("scope") or action.get("scope") == "NONE":
                 if "相手" not in target_str: target_str = "相手の" + target_str
            ...
            for i, opt_chain in enumerate(options):
                ...
                for a in opt_chain:
                    if isinstance(a, dict) and (
                        ...
                        parts.append(cls.format_command(a, is_spell=is_spell, sample=sample))
                    else:
            ...
            return "\n".join(lines)
             ...
             if stat_name:
                 ...
                 if input_key:
                     ...
                     if usage_label: base += f"（{usage_label}）"
                 return base
             ...
             if str(mode) == "CARDS_MATCHING_FILTER" or str(mode) == "COUNT_CARDS" or not mode:
                 ...
                 if target_str and target_str != "カード":
                     ...
                     if len(zone_names) == 1: base = f"{zone_names[0]}のカードの枚数を数える。"
                     else: base = f"{'または'.join(zone_names)}のカードの枚数を数える。"
                 else:
                     base = "カードの数を数える。"
             ...
             elif ftype == "TURN_CHANGE": return f"ターンを終了する。"
             elif ftype == "SET_ACTIVE_PLAYER": return f"手番を変更する。"
             return f"進行制御({tr(ftype)}): {val1}"

        elif atype == "GAME_RESULT":
             res = action.get("result", "")
             if not res: res = action.get("str_val") or action.get("str_param", "")
            ...
            return f"{target_str}をカードの下に重ねる。"
             ...
             amt = val1 if val1 > 0 else 1
             if amt == 1: return f"{target_str}をカードの下に重ねる。"
             ...
             if scope in ["PLAYER_OPPONENT", "OPPONENT"]: target_str_lock = "相手"
             elif scope in ["PLAYER_SELF", "SELF"]: target_str_lock = "自分"
             elif scope == "ALL_PLAYERS": target_str_lock = "すべてのプレイヤー"
             else: target_str_lock, _ = cls._resolve_target(action, is_spell)
             duration = val1 if val1 > 0 else 1
            ...
            if isinstance(sel, int) and sel >= 0: return f"選択肢{sel}を確定する。"
            indices = action.get("selected_indices") or []
            if isinstance(indices, list) and indices: return f"選択（{indices}）を確定する。"
            return "選択を確定する。"

        elif atype == "DECLARE_REACTION":
            if action.get("pass"): return "リアクション: パスする。"
            idx = action.get("reaction_index")
            if isinstance(idx, int): return f"リアクションを宣言する（インデックス {idx}）。"
            return "リアクションを宣言する。"
            ...
            if key:
                ...
                if stat_name: return f"統計更新: {stat_name} += {amount}"
            ...
            if stat_name:
                if sample is not None:
                    try:
                        ...
                        if val is not None: return f"{stat_name}（例: {val}{stat_unit}）"
            ...
            if input_key: return f"{deck_owner}山札の上から、その数だけ表向きにする。"
            return f"{deck_owner}山札の上から{val1}枚を表向きにする。"

        elif atype == "COUNT_CARDS":
            if not target_str or target_str == "カード": return f"({tr('COUNT_CARDS')})"
    ...
    def _format_zone_move_command(cls, atype: str, action: Dict[str, Any], is_spell: bool, val1: int, target_str: str) -> str:
        ...
        if atype == "TRANSITION":
            ...
            if from_z == "BATTLE_ZONE" and to_z == "GRAVEYARD":
                if up_to_flag and amount > 0: template = "{from_z}の{target}を{amount}{unit}まで選び、{to_z}に置く。"
                else:
                    template = "{from_z}の{target}を{amount}{unit}{to_z}に置く。"
                    if amount == 0 and not input_key: template = "{from_z}の{target}をすべて{to_z}に置く。"
                ...
                if up_to_flag and amount > 0: template = "{from_z}の{target}を{amount}{unit}まで選び、{to_z}に戻す。"
                else:
                    template = "{from_z}の{target}を{amount}{unit}{to_z}に戻す。"
                    if amount == 0 and not input_key: template = "{from_z}の{target}をすべて{to_z}に戻す。"
            elif from_z == "HAND" and to_z == "MANA_ZONE":
                if up_to_flag and amount > 0: template = "{from_z}の{target}を{amount}{unit}まで選び、{to_z}に置く。"
                else: template = "{from_z}の{target}を{amount}{unit}{to_z}に置く。"
            elif from_z == "DECK" and to_z == "HAND":
                if up_to_flag:
                    if target_str != "カード": template = "{from_z}から{target}を{amount}{unit}まで選び、{to_z}に加える。"
                    else: template = "山札からカードを最大{amount}枚まで手札に加える。"
                else:
                    if target_str != "カード": template = "{from_z}から{target}を{amount}{unit}選び、{to_z}に加える。"
                    else: template = "山札からカードを{amount}枚手札に加える。"
            elif from_z == "GRAVEYARD" and to_z == "HAND":
                if up_to_flag and amount > 0: template = "{from_z}の{target}を{amount}{unit}まで選び、{to_z}に戻す。"
                else: template = "{from_z}の{target}を{amount}{unit}{to_z}に戻す。"
            elif from_z == "GRAVEYARD" and to_z == "BATTLE_ZONE":
                if up_to_flag and amount > 0: template = "{from_z}の{target}を{amount}{unit}まで選び、{to_z}に出す。"
                else: template = "{from_z}の{target}を{amount}{unit}{to_z}に出す。"
            elif to_z == "GRAVEYARD":
                if up_to_flag and amount > 0: template = "{from_z}の{target}を{amount}{unit}まで選び、{to_z}に置く。"
                else: template = "{from_z}の{target}を{amount}{unit}{to_z}に置く。"
            elif to_z == "DECK_BOTTOM":
                if input_key:
                    if up_to_flag: template = "{from_z}の{target}をその同じ数だけまで選び、{to_z}に置く。"
                    else: template = "{from_z}の{target}をその同じ数だけ選び、{to_z}に置く。"
                else:
                    if up_to_flag and amount > 0: template = "{from_z}の{target}を{amount}{unit}まで選び、{to_z}に置く。"
                    else: template = "{from_z}の{target}を{amount}{unit}{to_z}に置く。"

            if "{from_z}" in template: template = template.replace("{from_z}", cls._zone_to_japanese(from_z))
            if "{to_z}" in template: template = template.replace("{to_z}", cls._zone_to_japanese(to_z))

            return template

        elif atype == "REPLACE_CARD_MOVE":
            dest_zone = action.get("destination_zone", "")
            if not dest_zone: dest_zone = action.get("to_zone", "DECK_BOTTOM")
            src_zone = action.get("source_zone", "")
            if not src_zone: src_zone = action.get("from_zone", "GRAVEYARD")
            ...
            template = ""
            if dest_zone == "HAND":
                if up_to_flag and val1 > 0: template = f"{{target}}を{{value1}}{{unit}}まで選び、{zone_str}に戻す。"
                else: template = f"{{target}}を{{value1}}{{unit}}選び、{zone_str}に戻す。"
                if src_str: template = src_str + "の" + template
                if is_all: template = f"{{target}}をすべて{zone_str}に戻す。"
            elif dest_zone == "MANA_ZONE":
                if up_to_flag and val1 > 0: template = f"{{target}}を{{value1}}{{unit}}まで選び、{zone_str}に置く。"
                else: template = f"{{target}}を{{value1}}{{unit}}選び、{zone_str}に置く。"
                if src_str: template = src_str + "の" + template
                if is_all: template = f"{{target}}をすべて{zone_str}に置く。"
    ...
    def _format_action(cls, action: Dict[str, Any], is_spell: bool = False, sample: List[Any] = None, card_mega_last_burst: bool = False) -> str:
        ...
        if atype == 'DRAW_CARD':
            if bool(action.get('up_to', False)): template = "最大{value1}枚まで引く。"
        ...
        if atype == 'TRANSITION':
            ...
            if alias:
                 if alias == "破壊":
                      ...
                      target_str, unit = cls._resolve_target(action, is_spell)
                      if up_to and amt > 0: t = f"{target_str}を{amt}{unit}まで選び、手札に戻す。"
                      elif amt == 0: t = f"{target_str}をすべて手札に戻す。"
                      else: t = f"{target_str}を{amt}{unit}手札に戻す。"
                      if bool(action.get("optional", False)):
                          if t.endswith("す。"): t = t[:-2] + "してもよい。"
                          else: t = t[:-1] + "てもよい。"
                      return t
                 elif alias == "マナチャージ": return f"自分の山札の上から{amt}枚をマナゾーンに置く。"
                 elif alias == "シールド焼却": return f"相手のシールドを{amt}つ選び、墓地に置く。"

            if (from_zone == 'DECK' or from_zone == '') and to_zone == 'HAND':
                if not amt and isinstance(action.get('target_filter'), dict): amt = action.get('target_filter', {}).get('count', 1)
                if up_to: return f"山札からカードを最大{amt}枚まで手札に加える。"
                else: return f"カードを{amt}枚引く。"
        ...
        if input_key:
            ...
            if input_usage:
                ...
                if label: usage_label_suffix = f"（{label}）"

            if atype == "DRAW_CARD":
                up_to_flag = bool(action.get('up_to', False))
                template = f"カードをその同じ枚数引く。{usage_label_suffix}"
                if up_to_flag: template = f"カードをその同じ枚数まで引く。{usage_label_suffix}"
            elif atype == "DESTROY":
                up_to_flag = bool(action.get('up_to', False))
                if up_to_flag: template = f"{{target}}をその同じ数だけまで選び、破壊する。{usage_label_suffix}"
                else: template = f"{{target}}をその同じ数だけ破壊する。{usage_label_suffix}"
            elif atype == "TAP":
                up_to_flag = bool(action.get('up_to', False))
                if up_to_flag: template = f"{{target}}をその同じ数だけまで選び、タップする。{usage_label_suffix}"
                else: template = f"{{target}}をその同じ数だけ選び、タップする。{usage_label_suffix}"
            elif atype == "UNTAP":
                up_to_flag = bool(action.get('up_to', False))
                if up_to_flag: template = f"{{target}}をその同じ数だけまで選び、アンタップする。{usage_label_suffix}"
                else: template = f"{{target}}をその同じ数だけ選び、アンタップする。{usage_label_suffix}"
            elif atype == "RETURN_TO_HAND":
                up_to_flag = bool(action.get('up_to', False))
                if up_to_flag: template = f"{{target}}をその同じ数だけまで選び、手札に戻す。{usage_label_suffix}"
                else: template = f"{{target}}をその同じ数だけ選び、手札に戻す。{usage_label_suffix}"
            elif atype == "SEND_TO_MANA":
                up_to_flag = bool(action.get('up_to', False))
                if up_to_flag: template = f"{{target}}をその同じ数だけまで選び、マナゾーンに置く。{usage_label_suffix}"
                else: template = f"{{target}}をその同じ数だけ選び、マナゾーンに置く。{usage_label_suffix}"
            elif atype == "TRANSITION":
                val1 = "その同じ枚数"
                if bool(action.get('up_to', False)): val1 = "その同じ枚数まで"
            elif atype == "MOVE_CARD":
                dest_zone = action.get("destination_zone", "")
                up_to_flag = bool(action.get('up_to', False))
                if dest_zone == "DECK_BOTTOM":
                    if up_to_flag: template = f"{{target}}をその同じ数だけまで選び、山札の下に置く。{usage_label_suffix}"
                    else: template = f"{{target}}をその同じ数だけ選び、山札の下に置く。{usage_label_suffix}"
                elif dest_zone == "GRAVEYARD":
                    if up_to_flag: template = f"{{target}}をその同じ数だけまで選び、墓地に置く。{usage_label_suffix}"
                    else: template = f"{{target}}をその同じ数だけ選び、墓地に置く。{usage_label_suffix}"
                elif dest_zone == "HAND":
                    if up_to_flag: template = f"{{target}}をその同じ数だけまで選び、手札に戻す。{usage_label_suffix}"
                    else: template = f"{{target}}をその同じ数だけ選び、手札に戻す。{usage_label_suffix}"
                elif dest_zone == "MANA_ZONE":
                ...
                if up_to_discard: template = f"手札をその同じ枚数まで捨てる。{usage_label_suffix}"
                else: template = f"手札をその同じ枚数捨てる。{usage_label_suffix}"
            else:
                val1 = "その数"
        ...
        if atype == "DISCARD":
            ...
            up_to_discard = bool(action.get('up_to', False))
            if amt == 0: template = "手札をすべて捨てる。"
            elif up_to_discard: template = f"手札を{amt}枚まで捨てる。"
            else: template = f"手札を{amt}枚捨てる。"
            return template
        ...
        if not template: return f"({tr(atype)})"
        ...
        if atype == "GRANT_KEYWORD" or atype == "ADD_KEYWORD":
            ...
            mega_burst_prefix = ""
            if is_mega_last_burst: mega_burst_prefix = "このクリーチャーがバトルゾーンから離れて、"
            ...
            if "SPELL" in types or not types:
                ...
                if zones:
                    ...
                    if len(zone_names) == 1: zone_phrase = zone_names[0] + "から"
                    else: zone_phrase = "または".join(zone_names) + "から"

                tf_no_zones = temp_filter.copy()
                if "zones" in tf_no_zones: tf_no_zones["zones"] = []
            ...
            if not action.get("source_zone") and "zones" in temp_filter:
                ...
                if len(zones) == 1: action["source_zone"] = zones[0]

            if action.get("value1", 0) == 0:
                max_cost = temp_filter.get("max_cost", 999)
                if isinstance(max_cost, dict): pass
                elif max_cost < 999:
            ...
            verb = "プレイする"
            types = temp_filter.get("types", [])
            if "SPELL" in types and "CREATURE" not in types: verb = "唱える"
            elif "CREATURE" in types: verb = "召喚する"

            play_flags = action.get("play_flags")
            is_free = False
            if isinstance(play_flags, bool) and play_flags: is_free = True
            elif isinstance(play_flags, list) and ("FREE" in play_flags or "COST_FREE" in play_flags): is_free = True

            if is_free: verb = f"コストを支払わずに{verb}"
            ...
            if use_linked_cost:
                if action.get("source_zone"): template = "{source_zone}から{target}を" + verb + f"。{usage_label_suffix}"
                else: template = "{target}を" + verb + f"。{usage_label_suffix}"
            else:
                if action.get("source_zone"): template = "{source_zone}からコスト{value1}以下の{target}を" + verb + f"。{usage_label_suffix}"
                else: template = "コスト{value1}以下の{target}を" + verb + f"。{usage_label_suffix}"
        ...
        if "{filter}" in text: text = text.replace("{filter}", target_str)
        ...
        if optional:
            if text.endswith("する。"): text = text[:-3] + "してもよい。"
            elif text.endswith("く。"): text = text[:-2] + "いてもよい。"
            elif text.endswith("す。"): text = text[:-2] + "してもよい。"
            elif text.endswith("る。"): text = text[:-2] + "てもよい。"
            elif text.endswith("う。"): text = text[:-2] + "ってもよい。"
            else:
                if not text.endswith("てもよい。"): text = text[:-1] + "てもよい。"
    ...
    def _resolve_target(cls, action: Dict[str, Any], is_spell: bool = False) -> Tuple[str, str]:
        ...
        if filter_def.get('is_trigger_source'):
            types = filter_def.get('types', [])
            if 'SPELL' in types: return ("その呪文", "枚")
            elif 'CARD' in types: return ("そのカード", "枚")
            return ("そのクリーチャー", "体")
        ...
        if atype == "DISCARD" and scope == "NONE": scope = "PLAYER_SELF"
        ...
        if filter_def:
            ...
            if not prefix and owner != "NONE":
                 ...
                 if owner_text: prefix = owner_text
            ...
            if isinstance(min_cost, dict):
                usage = min_cost.get("input_value_usage", "")
                if usage == "MIN_COST": adjectives += "コストその数以上の"
            elif min_cost > 0: adjectives += f"コスト{min_cost}以上の"
            elif has_input_key and input_usage == "MIN_COST": adjectives += "コストその数以上の"

            max_cost = filter_def.get("max_cost", 999)
            if max_cost is None: max_cost = 999
            if isinstance(max_cost, dict):
                usage = max_cost.get("input_value_usage", "")
                if usage == "MAX_COST": adjectives += "コストその数以下の"
            elif max_cost < 999: adjectives += f"コスト{max_cost}以下の"
            elif has_input_key and input_usage == "MAX_COST": adjectives += "コストその数以下の"

            min_power = filter_def.get("min_power", 0)
            if min_power is None: min_power = 0
            if isinstance(min_power, dict):
                usage = min_power.get("input_value_usage", "")
                if usage == "MIN_POWER": adjectives += "パワーその数以上の"
            elif min_power > 0: adjectives += f"パワー{min_power}以上の"
            elif has_input_key and input_usage == "MIN_POWER": adjectives += "パワーその数以上の"

            max_power = filter_def.get("max_power", 999999)
            if max_power is None: max_power = 999999
            if isinstance(max_power, dict):
                usage = max_power.get("input_value_usage", "")
                if usage == "MAX_POWER": adjectives += "パワーその数以下の"
            elif max_power < 999999: adjectives += f"パワー{max_power}以下の"
            elif has_input_key and input_usage == "MAX_POWER": adjectives += "パワーその数以下の"
            ...
            if atype == "SEARCH_DECK": zone_noun = ""
    ...
    def _merge_action_texts(cls, raw_items: List[Dict[str, Any]], formatted_texts: List[str]) -> str:
        ...
        if len(raw_items) >= 2 and is_cast_spell_item(raw_items[0]) and is_replace_card_move(raw_items[1]):
            ...
            if len(formatted_texts) > 2:
                ...
                if rest: merged = merged.rstrip('。') + '、' + rest
            return merged
    ...
    def generate_trigger_filter_description(cls, trigger_filter: Dict[str, Any]) -> str:
        ...
        if types:
            ...
            if type_names: descriptions.append("/".join(type_names))
        ...
        if races: descriptions.append("/".join(races))

        exact_cost = trigger_filter.get("exact_cost")
        cost_ref = trigger_filter.get("cost_ref")
        min_cost = trigger_filter.get("min_cost", 0) or 0
        max_cost = trigger_filter.get("max_cost", 999) if trigger_filter.get("max_cost") is not None else 999

        if cost_ref: descriptions.append("コスト【選択数字】")
        elif exact_cost is not None: descriptions.append(f"コスト{exact_cost}")
        else:
            if isinstance(min_cost, dict) and min_cost.get("input_value_usage") == "MIN_COST":
                ...
                if min_cost > 0 and max_cost < 999: descriptions.append(f"コスト{min_cost}～{max_cost}")
                elif min_cost > 0: descriptions.append(f"コスト{min_cost}以上")
                elif max_cost < 999: descriptions.append(f"コスト{max_cost}以下")

        min_power = trigger_filter.get("min_power", 0) or 0
        max_power = trigger_filter.get("max_power", 999999) if trigger_filter.get("max_power") is not None else 999999
        power_max_ref = trigger_filter.get("power_max_ref")

        if power_max_ref: descriptions.append("パワー【入力値】以下")
        else:
            if min_power > 0 and max_power < 999999: descriptions.append(f"パワー{min_power}～{max_power}")
            elif min_power > 0: descriptions.append(f"パワー{min_power}以上")
            elif max_power < 999999: descriptions.append(f"パワー{max_power}以下")
        ...
        if zones and "BATTLE_ZONE" not in zones:
            ...
            for z in zones:
                zone_text = cls._normalize_zone_name(z)
                if zone_text: zone_names.append(zone_text)
            if zone_names: descriptions.append("[" + "/".join(zone_names) + "]")
    ...
    def _compute_stat_from_sample(cls, key: str, sample: List[Any]) -> Any:
        ...
        if key == "MANA_CIVILIZATION_COUNT":
            civs = set()
            for entry in sample:
                if isinstance(entry, str): civs.add(entry)
                elif isinstance(entry, dict):
                    for c in entry.get('civilizations', []): civs.add(c)
            return len(civs)
        ...
        if key in count_stats: return len(sample)
        return None
