...
def map_action(action_data: Any) -> Dict[str, Any]:
    ...
    zone_map = {
        # Map legacy/full zone names to normalized short names expected by tests
        "MANA_ZONE": "MANA",
        "BATTLE_ZONE": "BATTLE",
        "SHIELD_ZONE": "SHIELD",
        "GRAVEYARD": "GRAVEYARD",
    ...
    if act_type == "MOVE_CARD":
        ...
        # Preserve explicit DRAW_CARD type for compatibility with tests
        cmd['type'] = "DRAW_CARD"
...
def _handle_specific_moves(act_type, act, cmd, src):
    # Preserve the original action type for clearer semantics in tests
    cmd['type'] = act_type
